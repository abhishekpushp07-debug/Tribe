#!/usr/bin/env python3
"""
BATCH 3: Stories (30 endpoints) + Reels (30 endpoints) Comprehensive Test
Testing EVERY endpoint specified in the review request
NO /auth/logout calls - tokens must remain alive
"""
...
def main():
    print("🚀 BATCH 3: STORIES + REELS COMPREHENSIVE TEST")
    print(f"📡 Testing API at: {BASE_URL}")
    print("⚠️  CRITICAL: NO /auth/logout calls - tokens stay alive throughout")
    
    # Get fresh tokens for both admin and user
    print("\n🔐 Getting authentication tokens...")
    token1, user1_id = get_fresh_token(ADMIN_CREDS)  # Admin token
    token2, user2_id = get_fresh_token(USER_CREDS)   # User token
    ...
    story2_result = test_endpoint(
        ...
        {"storyType":"TEXT","text":"Test interactions","background":"#2196f3"},
        [200, 201]
    )
    ...
    if story_id2:
        ...
        # 8. React to story (token2)
        result = test_endpoint("8. POST /api/stories/{storyId2}/react", "POST", f"/stories/{story_id2}/react", token2, {"reaction":"🔥"}, [200, 201])
        results.append(result)
    ...
    # 17. Create highlight
    if story_id2:
        highlight_result = test_endpoint("17. POST /api/me/highlights", "POST", "/me/highlights", token1, {"title":"Test Highlight","storyIds":[story_id2]}, [200, 201])
    ...
    # 19. Update highlight
    if highlight_id:
        result = test_endpoint("19. PATCH /api/me/highlights/{highlightId}", "PATCH", f"/me/highlights/{highlight_id}", token1, {"title":"Updated"}, [200, 204])
        results.append(result)
    ...
    result = test_endpoint("22. PATCH /api/me/story-settings", "PATCH", "/me/story-settings", token1, {"hideStoryFrom":[],"allowReplies":"EVERYONE"}, [200, 204])
    results.append(result)
    
    # 23. Get SSE stream (just check response, don't wait)
    result = test_endpoint("23. GET /api/stories/events/stream", "GET", "/stories/events/stream", token1, None, [200])
    results.append(result)
    ...
    # ============================================================================
    # REELS ENDPOINTS (30 endpoints total) 
    # ============================================================================
    ...
    if reel_id:
        ...
        # 42. Watch reel (token2)
        result = test_endpoint("42. POST /api/reels/{reelId}/watch", "POST", f"/reels/{reel_id}/watch", token2, {"watchDuration":15,"totalDuration":30}, [200, 201])
        results.append(result)
        ...
        # 44. Publish reel (token1, owner)
        result = test_endpoint("44. POST /api/reels/{reelId}/publish", "POST", f"/reels/{reel_id}/publish", token1, None, [200, 201, 204])
        results.append(result)
    ...
    # 50. Get audio detail
    result = test_endpoint("50. GET /api/reels/audio/{audioId}", "GET", "/reels/audio/Test Audio", token1, None, [200, 404])  # 404 acceptable if audio not found
    results.append(result)
    
    # 51. Create reel series
    series_result = test_endpoint("51. POST /api/me/reels/series", "POST", "/me/reels/series", token1, {"title":"Test Series","description":"batch test"}, [200, 201])
    ...
    # ============================================================================
    # PERFORMANCE ANALYSIS & CACHING TEST
    # ============================================================================
    
    print("\n⚡ PERFORMANCE ANALYSIS")
    print("="*60)
    
    # Test caching - call stories/feed twice to check for speed improvement
    ...
    # Categorize results
    stories_results = results[:25]  # First 25 are stories
    reels_results = results[25:]    # Rest are reels
    ...
    print("\n" + "="*80)
    print("📊 BATCH 3: STORIES + REELS COMPREHENSIVE TEST REPORT")
    print("="*80)
    ...
    print(f"   Requests >500ms: {slow_requests} (flagged with 🐌)")
    ...
    # Critical findings
    critical_issues = []
    if stories_success_rate < 50:
        critical_issues.append("Stories API severely impacted")
    if reels_success_rate < 50:
        critical_issues.append("Reels API severely impacted") 
    if slow_requests > total_tests * 0.3:
        critical_issues.append(f"Performance issues: {slow_requests} slow requests")
    ...
    if success_rate >= 85:
        print(f"\n🎉 BATCH 3 TESTING COMPLETED SUCCESSFULLY!")
        print(f"   All major Stories and Reels functionality is working excellently.")
        return True
    elif success_rate >= 70:
        print(f"\n✅ BATCH 3 TESTING COMPLETED WITH MINOR ISSUES")
        print(f"   Core functionality working but some endpoints need attention.")
        return True
    else:
        print(f"\n❌ BATCH 3 TESTING COMPLETED WITH SIGNIFICANT ISSUES")
        print(f"   Major functionality problems detected - requires investigation.")
        return False
