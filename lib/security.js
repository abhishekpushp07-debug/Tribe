...
export function getEndpointTier(route, method) {
  ...
  // Read operations (with specific heavy-endpoint throttling)
  if (route.startsWith('/tribes/leaderboard')) return { windowMs: 60_000, max: 20, name: 'LEADERBOARD', redisDownPolicy: 'DEGRADED' }
...
async function initRateLimitRedis() {
  ...
  // Skip Redis entirely if no REDIS_URL configured
  ...
  try {
    ...
    rlRedis.on('error', (err) => {
      rlRedisReady = false
      // Suppress noisy connection refused errors in non-Redis environments
    })
    rlRedis.on('close', () => {
      if (rlRedisReady) {
        rlRedisReady = false
...
// Strips MongoDB operator keys ($gt, $ne, $regex, etc.) from user-provided objects
export function stripNoSQLOperators(obj) {
  ...
  if (typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (key.startsWith('$')) continue // Strip all $-prefixed keys
      clean[key] = stripNoSQLOperators(value)
    }
    return clean
  }
  return obj
...
export function deepSanitizeStrings(obj) {
  ...
  if (obj !== null && typeof obj === 'object') {
    const result = {}
    for (const [key, value] of Object.entries(obj)) {
      if (key.startsWith('$')) continue // NoSQL injection prevention: strip $-prefixed keys
