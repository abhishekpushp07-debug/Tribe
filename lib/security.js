...
export function checkTieredRateLimit(ip, userId, tier) {
  ...
  // Per-IP check (only if ip is provided)
  if (ip) {
  ...
  // Per-user check (only if userId is provided)
  if (userId) {
...
// ========== INPUT SANITIZATION ==========
// For a social media API, user text fields should be plain text — no HTML.
...
const ALL_HTML_TAGS = /<\/?[a-z][a-z0-9]*[^>]*>/gi    // strip ALL HTML tags
const EVENT_HANDLER_PATTERN = /\bon\w+\s*=[^\s>]*/gi    // onerror=alert(1) — quoted or unquoted
...
export function sanitizeTextInput(text) {
  ...
  return text
    ...
    .replace(STYLE_BLOCK_PATTERN, '')       // Remove <style>...</style> blocks entirely
    .replace(ALL_HTML_TAGS, '')             // Remove ALL remaining HTML tags
    .replace(EVENT_HANDLER_PATTERN, '')     // Remove event handlers (even outside tags)
...
// Used at router level to sanitize ALL JSON request bodies centrally
