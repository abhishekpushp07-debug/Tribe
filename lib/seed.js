...
export async function autoSeed(db) {
  try {
    ...
    // Minimal valid MP4 (ftyp + mdat) for playback
    const SAMPLE_VIDEO_B64 = 'AAAAIGZ0eXBpc29tAAACAGlzb21pc28yYXZjMW1wNDEAAAAIbWRhdA=='
    ...
    // 7. Fix any test reels with example.com placeholder URLs
    const sharedMediaId = uuidv4()
    const existingBadReels = await db.collection('reels').countDocuments({
      playbackUrl: { $regex: 'example\\.com|ex\\.com' }
    })
    if (existingBadReels > 0) {
      await db.collection('media_assets').insertOne({
        id: sharedMediaId,
        ownerId: 'system',
        type: 'VIDEO',
      ...
      await db.collection('reels').updateMany(
        { playbackUrl: { $regex: 'example\\.com|ex\\.com' } },
        { $set: { playbackUrl: `/api/media/${sharedMediaId}`, mediaStatus: 'READY' } }
      )
      console.log(`[AUTO-SEED] Fixed ${existingBadReels} reels with example.com URLs`)
    }
