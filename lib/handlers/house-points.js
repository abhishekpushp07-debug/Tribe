...
export async function handleHousePoints(path, method, request, db) {
  ...
  if (route === 'house-points/award' && method === 'POST') {
    ...
    const effectiveGroupId = targetUser.tribeId || targetUser.houseId
    if (!effectiveGroupId) return { error: 'User has no tribe assigned', code: ErrorCode.VALIDATION, status: 400 }

    const entry = await awardHousePoints(db, userId, effectiveGroupId, points > 0 ? 'ADMIN_AWARD' : 'ADMIN_DEDUCT', points, { adminId: user.id, reason })

    await writeAudit(db, 'HOUSE_POINTS_AWARDED', user.id, 'USER', userId, { points, reason, tribeId: effectiveGroupId })
  ...
  if (route === 'house-points/leaderboard' && method === 'GET') {
    ...
    const leaderboard = await Promise.all(houses.map(async (h, rank) => {
      ...
      const memberCount = await db.collection('users').countDocuments({ $or: [{ houseId: h.id }, { tribeId: h.id }] })
