...
export async function handleAdmin(path, method, request, db) {
  ...
  if (route === 'appeals' && method === 'GET') {
    ...
    const appeals = await db.collection('appeals')
      ...
      .limit(20)
      .toArray()
    const cleanAppeals = appeals.map(a => { const { _id, ...rest } = a; return rest })
    return { data: { items: cleanAppeals, appeals: cleanAppeals, count: cleanAppeals.length } }
  }
  ...
  // ========================
  // POST /admin/colleges/seed — Stage 2: ADMIN auth required
  // ========================
  if (route === 'admin/colleges/seed' && method === 'POST') {
    ...
    try {
      ...
      // Expected: duplicate key errors when re-seeding. Log but don't fail.
      const { default: logger } = await import('@/lib/logger')
      logger.warn('ADMIN', 'college_seed_partial', { error: err.message, attempted: docs.length })
  ...
  if (route === 'grievances' && method === 'GET') {
    ...
    return { data: { items: cleaned, grievances: cleaned, tickets: cleaned, count: cleaned.length } }
  }
