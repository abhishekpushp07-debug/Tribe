...
export async function handleAdmin(path, method, request, db) {
  ...
  if (route === 'admin/stats' && method === 'GET') {
    const [users, posts, reels, stories, colleges, houses, openReports, pendingAppeals] = await Promise.all([
      ...
      db.collection('content_items').countDocuments({ kind: 'STORY', visibility: 'PUBLIC' }),
    ...
    return { data: { users, posts, reels, stories, colleges, houses, openReports, pendingAppeals } }
  }
  ...
  if (route === 'grievances' && method === 'POST') {
    ...
    return { data: { ticket: clean }, status: 201 }
  }
  ...
  if (route === 'grievances' && method === 'GET') {
    ...
    const tickets = await db.collection('grievance_tickets')
      ...
      .limit(20)
      .toArray()
    return { data: { tickets: tickets.map(t => { const { _id, ...rest } = t; return rest }) } }
  }
