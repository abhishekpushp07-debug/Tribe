...
export async function interceptFollowForPrivateAccount(path, method, request, db) {
  ...
  // Block check: don't allow follow requests to/from blocked users
  const blockExists = await db.collection('blocks').findOne({
    $or: [
      { blockerId: user.id, blockedId: targetId },
      { blockerId: targetId, blockedId: user.id },
    ],
  })
  if (blockExists) return { error: 'Cannot send follow request', code: 'BLOCKED', status: 403 }

  // Rate limit: max 30 follow requests per hour
  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000)
  const recentRequests = await db.collection('follow_requests').countDocuments({
    requesterId: user.id, createdAt: { $gte: oneHourAgo },
  })
  if (recentRequests >= 30) {
    return { error: 'Too many follow requests. Try again later.', code: 'RATE_LIMITED', status: 429 }
  }
