...
// Helper: fill time-series gaps with zeros for consistent charting
function fillTimeSeriesGaps(data, since, periodDays, valueKey = 'count') {
  const dateMap = new Map(data.map(d => [d._id || d.date, d[valueKey] || d.count || 0]))
  const filled = []
  for (let i = 0; i < periodDays; i++) {
    const date = new Date(since.getTime() + i * 24 * 60 * 60 * 1000)
    const key = date.toISOString().slice(0, 10)
    filled.push({ date: key, [valueKey]: dateMap.get(key) || 0 })
  }
  return filled
}
...
export async function handleAnalytics(path, method, request, db) {
  ...
  if (route === 'analytics/track' && method === 'POST') {
    ...
    if (eventType === 'PROFILE_VISIT' && targetId) {
      // Deduplicate unique visitors per day using upsert with conditional increment
      const visitUpdate = { $inc: { visitCount: 1 }, $setOnInsert: { profileId: targetId, day: event.day, createdAt: new Date() } }
      if (currentUser?.id) {
        // Track unique visitors using addToSet
        visitUpdate.$addToSet = { uniqueVisitorIds: currentUser.id }
      }
  ...
  if (route === 'analytics/overview' && method === 'GET') {
    ...
    // Cache per user+period (60s TTL — analytics don't need to be real-time)
    const analyticsCacheKey = `${user.id}:${period}`
    const cachedAnalytics = await cache.get(CacheNS.ANALYTICS_OVERVIEW, analyticsCacheKey)
    if (cachedAnalytics) return { data: cachedAnalytics }
    ...
    await cache.set(CacheNS.ANALYTICS_OVERVIEW, analyticsCacheKey, overviewResult, CacheTTL.ANALYTICS_OVERVIEW)
    return { data: overviewResult }
  }
  ...
  if (route === 'analytics/reach' && method === 'GET') {
    ...
    return {
      data: {
        period,
        profileVisits: {
          byDay: profileVisitsByDay.map(d => ({ date: d.day, visits: d.visitCount || 0, uniqueVisitors: (d.uniqueVisitorIds || []).length })),
  ...
  // ========================
  // GET /analytics/stories — Story performance analytics
  // ========================
  if (route === 'analytics/stories' && method === 'GET') {
    const user = await requireAuth(request, db)
    const url = new URL(request.url)
    const period = url.searchParams.get('period') || '30d'
    const periodDays = period === '7d' ? 7 : period === '30d' ? 30 : 90
    ...
    const stories = await db.collection('stories')
      .find({ authorId: user.id, createdAt: { $gte: since } })
      .sort({ createdAt: -1 })
      .limit(50)
      .toArray()
    ...
    const [totalViews, totalReactions, viewsByDay] = await Promise.all([
      storyIds.length > 0 ? db.collection('story_views').countDocuments({ storyId: { $in: storyIds } }) : 0,
      storyIds.length > 0 ? db.collection('story_reactions').countDocuments({ storyId: { $in: storyIds } }) : 0,
      storyIds.length > 0 ? db.collection('story_views').aggregate([
    ...
    return {
      data: {
        ...
        viewsByDay: fillTimeSeriesGaps(viewsByDay, since, periodDays, 'count'),
        stories: stories.slice(0, 20).map(s => {
          ...
          return {
            ...
            reactionCount: clean.reactionCount || 0,
          }
        }),
      },
    }
