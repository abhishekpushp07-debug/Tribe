...
async function canViewStory(db, story, viewerId) {
  ...
  if (isAdmin({ role: 'check' }) === false) { /* placeholder */ }
...
export async function handleStories(path, method, request, db) {
  ...
  if (path[0] === 'stories' && path.length === 3 && path[2] === 'reply' && method === 'POST') {
    ...
    await db.collection('stories').updateOne({ id: storyId }, { $inc: { replyCount: 1 } })
...
async function getStickerResults(db, storyId, stickerId, sticker) {
  ...
  switch (sticker.type) {
    case 'POLL': {
      const voteCounts = new Array(sticker.options.length).fill(0)
      for (const r of responses) {
        if (r.response?.optionIndex !== undefined) voteCounts[r.response.optionIndex]++
      }
      return {
        ...
        options: sticker.options.map((opt, i) => ({
          text: opt,
          votes: voteCounts[i],
          percentage: totalResponses > 0 ? Math.round((voteCounts[i] / totalResponses) * 100) : 0,
        })),
      }
    }
    case 'QUIZ': {
      const answerCounts = new Array(sticker.options.length).fill(0)
      let correctCount = 0
      for (const r of responses) {
        if (r.response?.optionIndex !== undefined) {
          answerCounts[r.response.optionIndex]++
          if (r.response.correct) correctCount++
        }
      }
      return {
        ...
        options: sticker.options.map((opt, i) => ({
          ...
          percentage: totalResponses > 0 ? Math.round((answerCounts[i] / totalResponses) * 100) : 0,
        })),
      }
    }
    case 'QUESTION': {
    ...
    case 'EMOJI_SLIDER': {
      const values = responses.map(r => r.response?.value).filter(v => v !== undefined)
