  ...
  async function runCleanup() {
    try {
      ...
      if (expiredStories.length > 0) {
        ...
        logger.info('SYSTEM', 'story_expiry_complete', { expired: expiredStories.length, archived: archiveIds.length, notArchived: noArchiveIds.length })
      }
    } catch (err) {
      const { default: logger } = await import('@/lib/logger')
      logger.error('SYSTEM', 'story_expiry_error', { error: err.message })
    }
  }
