...
// ========== STORY EXPIRY WORKER ==========
// Runs every 30 minutes, marks expired ACTIVE stories as EXPIRED, respects autoArchive
let expiryWorkerStarted = false

function startExpiryWorker(db) {
  ...
  async function runCleanup() {
    try {
      ...
      // Find all expired active stories
      ...
      // Emit real-time expired events per author
      const authorGroups = {}
      for (const s of expiredStories) {
      ...
      for (const [authorId, storyIds] of Object.entries(authorGroups)) {
        await publishStoryEvent(authorId, {
          type: StoryEventType.EXPIRED,
          storyIds,
      ...
      if (expiredStories.length > 0) {
        console.log(`[StoryExpiry] Marked ${expiredStories.length} stories as EXPIRED (${archiveIds.length} archived, ${noArchiveIds.length} not archived)`)
      }
    } catch (err) {
      console.error('[StoryExpiry] Cleanup error:', err.message)
    }
  }
...
async function getBlockedUserIds(db, viewerId, candidateUserIds) {
...
async function canViewStory(db, story, viewerId) {
  ...
  // Block check: bidirectional block overrides everything
...
async function canInteractWithStory(db, story, userId) {
...
export async function handleStories(path, method, request, db) {
  ...
  // Start expiry worker on first request (lazy init)
  startExpiryWorker(db)
  ...
  if (route === 'stories/feed' && method === 'GET') {
    ...
    // Batch block check: exclude blocked users from rail
    const blockedUserIds = await getBlockedUserIds(db, user.id, followeeIds)
    const eligibleAuthorIds = followeeIds.filter(id => !blockedUserIds.has(id))

    // Fetch active stories from eligible users
    const now = new Date()
  ...
  if (path[0] === 'users' && path.length === 3 && path[2] === 'highlights' && method === 'GET') {
    ...
    if (highlights.length === 0) {
      return { data: { highlights: [], total: 0 } }
    }
