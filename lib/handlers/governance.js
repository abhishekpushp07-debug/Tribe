...
export async function handleGovernance(path, method, request, db) {
  ...
  if (path[0] === 'governance' && path[1] === 'college' && path.length >= 4 && path[3] === 'applications' && method === 'GET') {
    ...
    return { data: { items: enriched, applications: enriched, count: enriched.length } }
  }
  ...
  if (path[0] === 'governance' && path[1] === 'applications' && path.length === 4 && path[3] === 'vote' && method === 'POST') {
    ...
    // Atomic vote: $addToSet ensures no duplicate, condition check in filter
    const voteField = vote === 'APPROVE' ? 'votes.approve' : 'votes.reject'
    const atomicResult = await db.collection('board_applications').findOneAndUpdate(
      { id: appId, status: 'PENDING', voters: { $ne: user.id } },
      { $addToSet: { voters: user.id }, $inc: { [voteField]: 1 } },
      { returnDocument: 'after' }
    )

    if (!atomicResult) {
      return { error: 'Already voted or application closed', code: ErrorCode.CONFLICT, status: 409 }
    }
    const updatedApp = atomicResult
  ...
  if (path[0] === 'governance' && path[1] === 'college' && path.length >= 4 && path[3] === 'proposals' && method === 'GET') {
    ...
    return { data: { items: enriched, proposals: enriched, count: enriched.length } }
  }
  ...
  if (path[0] === 'governance' && path[1] === 'proposals' && path.length === 4 && path[3] === 'vote' && method === 'POST') {
    ...
    // Atomic vote with race-condition protection
    const atomicResult = await db.collection('board_proposals').findOneAndUpdate(
      { id: proposalId, status: 'OPEN', voters: { $ne: user.id } },
      { $addToSet: { voters: user.id }, $inc: { [voteField]: 1 } },
      { returnDocument: 'after' }
    )

    if (!atomicResult) {
      return { error: 'Already voted or proposal closed', code: ErrorCode.CONFLICT, status: 409 }
    }
    ...
    const updatedProposal = atomicResult
