...
function sanitizeReel(reel, mediaAsset) {
  if (!reel) return null
  const { _id, ...clean } = reel
  const asset = mediaAsset || {}
  const av = asset.variants || {}

  const resolvedThumb = clean.thumbnailUrl || asset.thumbnailUrl || av.thumbnail?.url || ''
  const resolvedPoster = clean.posterFrameUrl || asset.posterFrameUrl || av.poster?.url || resolvedThumb
  const resolvedPlayback = asset.playbackUrl || av['720p']?.url || av['480p']?.url || av.faststart?.url || clean.playbackUrl || ''
  const resolvedVariants = (av['360p'] || av['480p'] || av['720p'] || av.hls || av.dash) ? {
    ...(av.original ? { original: av.original } : {}), ...(av['360p'] ? { '360p': av['360p'] } : {}),
    ...(av['480p'] ? { '480p': av['480p'] } : {}), ...(av['720p'] ? { '720p': av['720p'] } : {}),
    ...(av.faststart ? { faststart: av.faststart } : {}), ...(av.hls ? { hls: av.hls } : {}),
    ...(av.dash ? { dash: av.dash } : {}), ...(av.thumbnail ? { thumbnail: av.thumbnail } : {}),
    ...(av.poster ? { poster: av.poster } : {}),
  } : {}
  const bestHls = asset.hlsUrl || av.hls?.url || ''
  const bestDash = asset.dashUrl || av.dash?.url || ''
  const bestMp4 = av['720p']?.url || av['480p']?.url || av['360p']?.url || resolvedPlayback
  const mediaType = bestHls ? 'hls' : bestDash ? 'dash' : 'mp4'
  const isPlayable = !!bestMp4 && bestMp4 !== '' && clean.mediaStatus !== 'UPLOADING' && clean.mediaStatus !== 'FAILED'

  return {
    id: clean.id || '', creatorId: clean.creatorId || '', caption: clean.caption || '',
    hashtags: clean.hashtags || [], mentions: clean.mentions || [],
    visibility: clean.visibility || 'PUBLIC', status: clean.status || 'PUBLISHED',
    moderationStatus: clean.moderationStatus || 'APPROVED',
    playbackUrl: resolvedPlayback, thumbnailUrl: resolvedThumb, posterFrameUrl: resolvedPoster,
    hlsUrl: bestHls, dashUrl: bestDash, mediaId: clean.mediaId || '',
    mediaStatus: clean.mediaStatus || 'READY',
    playbackStatus: asset.playbackStatus || (clean.mediaStatus === 'READY' ? 'READY' : clean.mediaStatus || 'READY'),
    width: clean.width || asset.width || 0, height: clean.height || asset.height || 0,
    aspectRatio: clean.aspectRatio || '9:16',
    durationMs: clean.durationMs || (asset.duration ? asset.duration * 1000 : 0),
    likeCount: clean.likeCount || 0, commentCount: clean.commentCount || 0,
    saveCount: clean.saveCount || 0, shareCount: clean.shareCount || 0,
    viewCount: clean.viewCount || 0, uniqueViewerCount: clean.uniqueViewerCount || 0,
    impressionCount: clean.impressionCount || 0, completionCount: clean.completionCount || 0,
    avgWatchTimeMs: clean.avgWatchTimeMs || 0, replayCount: clean.replayCount || 0,
    reportCount: clean.reportCount || 0, score: clean.score || 0,
    likedByMe: clean.likedByMe || false, savedByMe: clean.savedByMe || false,
    hiddenByMe: clean.hiddenByMe || false, notInterestedByMe: clean.notInterestedByMe || false,
    pinnedToProfile: clean.pinnedToProfile || false,
    audioMeta: clean.audioMeta || {}, remixOf: clean.remixOf || {},
    syntheticDeclaration: clean.syntheticDeclaration || false,
    brandedContent: clean.brandedContent || false, collabCreators: clean.collabCreators || [],
    seriesId: clean.seriesId || '', seriesOrder: clean.seriesOrder ?? 0,
    authorType: clean.authorType || 'USER', pageId: clean.pageId || '',
    collegeId: clean.collegeId || '', tribeId: clean.tribeId || '',
    createdAt: clean.createdAt || '', updatedAt: clean.updatedAt || '', publishedAt: clean.publishedAt || '',
    variants: resolvedVariants, _feedScore: clean._feedScore, _feedRank: clean._feedRank,
    playbackMeta: {
      loop: true, preload: 'auto', playsInline: true, muted: true, crossOrigin: 'anonymous',
      durationMs: clean.durationMs || (asset.duration ? asset.duration * 1000 : 0),
      aspectRatio: clean.aspectRatio || '9:16', mediaType, isPlayable,
    },
    recommended: {
      dash: bestDash, hls: bestHls, mp4: bestMp4, poster: resolvedPoster,
      primaryUrl: bestHls || bestDash || bestMp4, fallbackUrl: bestMp4, mediaType, isPlayable,
    },
  }
}

function safeCreator(snippet) {
  if (!snippet) return { id: '', displayName: '', username: '', avatarUrl: '', profilePicUrl: '', role: 'USER', collegeId: '', collegeName: '', tribeId: '', tribeCode: '', tribeName: '', tribeHeroName: '' }
  return { id: snippet.id || '', displayName: snippet.displayName || '', username: snippet.username || '', avatarUrl: snippet.avatarUrl || '', profilePicUrl: snippet.profilePicUrl || '', avatarMediaId: snippet.avatarMediaId || '', role: snippet.role || 'USER', collegeId: snippet.collegeId || '', collegeName: snippet.collegeName || '', tribeId: snippet.tribeId || '', tribeCode: snippet.tribeCode || '', tribeName: snippet.tribeName || '', tribeHeroName: snippet.tribeHeroName || '' }
}

function sanitizeReels(reels, mediaAssetMap) {
  const m = mediaAssetMap || {}
  return reels.map(r => sanitizeReel(r, m[r.mediaId] || null)).filter(Boolean)
}
...
export async function handleReels(path, method, request, db) {
  ...
  if (path[0] === 'reels' && path.length === 1 && method === 'POST') {
    ...
    // Auto-resolve thumbnail from media asset if not provided
    const resolvedThumbnailUrl = thumbnailUrl || resolvedMediaAsset?.thumbnailUrl || null
    const resolvedPosterFrameUrl = posterFrameUrl || resolvedThumbnailUrl || null
    ...
    const reel = {
      ...
      posterFrameUrl: resolvedPosterFrameUrl,
      aspectRatio: resolvedMediaAsset ? (resolvedMediaAsset.width && resolvedMediaAsset.height ? `${resolvedMediaAsset.width}:${resolvedMediaAsset.height}` : '9:16') : '9:16',
      width: resolvedMediaAsset?.width || null,
      height: resolvedMediaAsset?.height || null,
      variants: variants || [],
      visibility,
      moderationStatus,
  ...
  if (path[0] === 'reels' && path[1] === 'feed' && path.length === 2 && method === 'GET') {
    ...
    const rawItems = reels.slice(0, limit)

    // Batch: resolve media assets for thumbnail/poster/variants
    const mediaIds = [...new Set(rawItems.map(r => r.mediaId).filter(Boolean))]
    const [creators, mediaAssets] = await Promise.all([
      (async () => {
        const cIds = [...new Set(rawItems.map(r => r.creatorId))]
        return cIds.length > 0 ? db.collection('users').find({ id: { $in: cIds } }).toArray() : []
      })(),
      mediaIds.length > 0
        ? db.collection('media_assets').find(
            { id: { $in: mediaIds } },
            { projection: { _id: 0, id: 1, publicUrl: 1, playbackUrl: 1, hlsUrl: 1, dashUrl: 1, thumbnailUrl: 1, posterFrameUrl: 1, playbackStatus: 1, variants: 1, duration: 1 } }
          ).toArray()
        : [],
    ])
    const mediaAssetMap = Object.fromEntries(mediaAssets.map(a => [a.id, a]))
    const items = sanitizeReels(rawItems, mediaAssetMap)

    // Batch: add creator info and viewer fields
    const creatorMap = Object.fromEntries(creators.map(u => [u.id, safeCreator(toUserSnippet(u))]))
    ...
    const nextCursor = hasMore && lastItem ? `${lastItem.score}|${lastItem.id}` : ''
  ...
  if (path[0] === 'reels' && path[1] === 'following' && path.length === 2 && method === 'GET') {
    ...
    if (eligibleIds.length === 0) {
      return { data: { items: [], pagination: { nextCursor: '', hasMore: false }, nextCursor: '' } }
    }
    ...
    return {
      data: {
        items: enriched,
        pagination: {
          nextCursor: hasMore ? (items[items.length - 1].publishedAt?.toISOString() || '') : '',
          hasMore,
        },
        nextCursor: hasMore ? (items[items.length - 1].publishedAt?.toISOString() || '') : '',
      },
  ...
  if (path[0] === 'users' && path.length === 3 && path[2] === 'reels' && method === 'GET') {
    ...
    return {
      data: {
        creator: safeCreator(toUserSnippet(creator)),
        items,
        pagination: {
          nextCursor: hasMore ? (items[items.length - 1].publishedAt?.toISOString() || '') : '',
          hasMore,
  ...
  if (path[0] === 'reels' && path.length === 2 && method === 'GET' && !['feed', 'following', 'audio', 'trending', 'personalized'].includes(path[1])) {
    ...
    // Get creator + resolve media asset for video pipeline data
    const [creator, mediaAsset] = await Promise.all([
      db.collection('users').findOne({ id: reel.creatorId }),
      reel.mediaId ? db.collection('media_assets').findOne(
        { id: reel.mediaId },
        { projection: { _id: 0, id: 1, publicUrl: 1, playbackUrl: 1, hlsUrl: 1, dashUrl: 1, thumbnailUrl: 1, posterFrameUrl: 1, playbackStatus: 1, variants: 1, duration: 1 } }
      ) : null,
    ])

    let result = { ...sanitizeReel(reel, mediaAsset), creator: safeCreator(toUserSnippet(creator)) }
    ...
    if (reel.remixOf?.reelId) {
      ...
      if (origReel) {
        ...
        result.remixSource = {
          reelId: origReel.id,
          creator: safeCreator(toUserSnippet(origCreator)),
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'comments' && method === 'GET') {
    ...
    const senders = senderIds.length > 0
      ...
      : []
    const senderMap = Object.fromEntries(senders.map(u => [u.id, safeCreator(toUserSnippet(u))]))
  ...
  if (path[0] === 'me' && path[1] === 'reels' && path[2] === 'archive' && path.length === 3 && method === 'GET') {
    ...
    return {
      data: {
        items,
        pagination: {
          nextCursor: hasMore ? (items[items.length - 1].archivedAt?.toISOString() || '') : '',
          hasMore,
        },
        nextCursor: hasMore ? (items[items.length - 1].archivedAt?.toISOString() || '') : '',
  ...
  if (path[0] === 'reels' && path[1] === 'trending' && path.length === 2 && method === 'GET') {
    ...
    const nextCursor = hasMore && lastItem ? `${lastItem.trendingScore}` : ''
  ...
  if (path[0] === 'reels' && path[1] === 'personalized' && path.length === 2 && method === 'GET') {
    ...
    const nextCursor = hasMore && lastItem ? (lastItem.publishedAt?.toISOString() || '') : ''
  ...
  if (path[0] === 'me' && path[1] === 'reels' && path[2] === 'analytics' && path[3] === 'detailed' && path.length === 4 && method === 'GET') {
    ...
    const engagerUsers = topEngagerIds.length > 0
      ...
      : []
    const engagerMap = Object.fromEntries(engagerUsers.map(u => [u.id, safeCreator(toUserSnippet(u))]))
