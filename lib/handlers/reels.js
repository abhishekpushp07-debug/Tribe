...
function sanitizeReel(reel, mediaAsset) {
  ...
  // Bridge: resolve thumbnail/poster/variants from media_assets pipeline
  const asset = mediaAsset || {}
  const av = asset.variants || {}

  // Resolve URLs with guaranteed fallback chain (NEVER null for mp4)
  const resolvedThumb = clean.thumbnailUrl || asset.thumbnailUrl || av.thumbnail?.url || null
  const resolvedPoster = clean.posterFrameUrl || asset.posterFrameUrl || av.poster?.url || resolvedThumb || clean.thumbnailUrl || null
  const resolvedPlayback = asset.playbackUrl || av['720p']?.url || av['480p']?.url || av.faststart?.url || clean.playbackUrl || null

  // Build variants object from media_assets pipeline
  const resolvedVariants = (av['360p'] || av['480p'] || av['720p'] || av.hls || av.dash) ? {
    ...(av.original ? { original: av.original } : {}),
    ...(av['360p'] ? { '360p': av['360p'] } : {}),
    ...(av['480p'] ? { '480p': av['480p'] } : {}),
    ...(av['720p'] ? { '720p': av['720p'] } : {}),
    ...(av.faststart ? { faststart: av.faststart } : {}),
    ...(av.hls ? { hls: av.hls } : {}),
    ...(av.dash ? { dash: av.dash } : {}),
    ...(av.thumbnail ? { thumbnail: av.thumbnail } : {}),
    ...(av.poster ? { poster: av.poster } : {}),
  } : (Array.isArray(clean.variants) && clean.variants.length === 0 ? {} : clean.variants)

  // Determine best playback URL for each protocol
  const bestHls = asset.hlsUrl || av.hls?.url || null
  const bestDash = asset.dashUrl || av.dash?.url || null
  const bestMp4 = av['720p']?.url || av['480p']?.url || av['360p']?.url || resolvedPlayback
  const bestPoster = resolvedPoster

  // Determine what media type ExoPlayer/frontend should use
  ...
  return {
    ...
    playbackMeta: {
      ...
      durationMs: clean.durationMs || (asset.duration ? asset.duration * 1000 : null),
      aspectRatio: clean.aspectRatio || '9:16',
      mediaType,        // "mp4" | "hls" | "dash" — tells ExoPlayer which decoder
      isPlayable,       // false = show "Processing" / don't attempt playback
    },
    // Android/iOS selection guide — GUARANTEED non-null mp4 when isPlayable
    recommended: {
      dash: bestDash,
      hls: bestHls,
      mp4: bestMp4,
      poster: bestPoster,
      // Explicit priority order for ExoPlayer
      primaryUrl: bestHls || bestDash || bestMp4,
      fallbackUrl: bestMp4,
      mediaType,
      isPlayable,
    },
...
export async function handleReels(path, method, request, db) {
  ...
  if (path[0] === 'users' && path.length === 3 && path[2] === 'reels' && method === 'GET') {
    ...
    return {
      data: {
        creator: creator ? toUserSnippet(creator) : null,
        items,
        pagination: {
          nextCursor: hasMore ? items[items.length - 1].publishedAt?.toISOString() : null,
          hasMore,
  ...
  if (path[0] === 'reels' && path.length === 2 && method === 'GET' && !['feed', 'following', 'audio', 'trending', 'personalized'].includes(path[1])) {
    ...
    let result = { ...sanitizeReel(reel, mediaAsset), creator: creator ? toUserSnippet(creator) : null }
    ...
    if (reel.remixOf?.reelId) {
      ...
      if (origReel) {
        ...
        result.remixSource = {
          reelId: origReel.id,
          creator: origCreator ? toUserSnippet(origCreator) : null,
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'comments' && method === 'GET') {
    ...
    const senders = senderIds.length > 0
      ...
      : []
    const senderMap = Object.fromEntries(senders.map(u => [u.id, toUserSnippet(u)]))
  ...
  if (path[0] === 'me' && path[1] === 'reels' && path[2] === 'analytics' && path[3] === 'detailed' && path.length === 4 && method === 'GET') {
    ...
    const engagerUsers = topEngagerIds.length > 0
      ...
      : []
    const engagerMap = Object.fromEntries(engagerUsers.map(u => [u.id, toUserSnippet(u)]))
