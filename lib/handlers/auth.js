...
import {
  generateSalt, hashPin, verifyPin, generateToken, sanitizeUser,
...
export async function handleAuth(path, method, request, db) {
  ...
  if (route === 'auth/register' && method === 'POST') {
    ...
    // Auto-assign tribe (Stage 12X: new users get tribe at signup)
    try {
      ...
      if (tribeDoc) {
        ...
        if (!membershipExists) {
          await db.collection('user_tribe_memberships').insertOne({
            ...
            legacyHouseId: houseDoc?.id || null,
            reassignmentCount: 0,
            auditRef: uuidv4(),
            createdAt: now,
            updatedAt: now,
      ...
      console.error('Auto tribe assignment failed:', e.message)
    }
