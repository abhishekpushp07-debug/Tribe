...
export async function handleSocial(path, method, request, db) {
  ...
  if (path[0] === 'follow' && path.length === 2 && method === 'POST') {
    ...
    if (targetId === user.id) {
      return { error: 'Cannot follow yourself', code: ErrorCode.VALIDATION, status: 400 }
    }
  ...
  if (path[0] === 'content' && path.length === 3 && path[2] === 'comments' && method === 'POST') {
    ...
    // AI moderation on comment text
    const modResult = await moderateContent(commentText)
    if (modResult.flagged && modResult.action === 'AUTO_REJECT') {
    ...
    const comment = {
      ...
      likeCount: 0,
      moderation: modResult.flagged ? { action: modResult.action, categories: modResult.categories, model: modResult.model } : null,
      createdAt: new Date(),
    }
