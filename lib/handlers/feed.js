...
export async function handleFeed(path, method, request, db) {
  ...
  if (route === 'feed/public' && method === 'GET') {
    ...
    if (cacheKey) {
      const cached = cache.get(CacheNS.PUBLIC_FEED, cacheKey)
    ...
    if (cacheKey) cache.set(CacheNS.PUBLIC_FEED, cacheKey, result, CacheTTL.PUBLIC_FEED)
    return { data: result }
  }
  ...
  if (path[0] === 'feed' && path[1] === 'college' && path.length === 3 && method === 'GET') {
    ...
    if (cacheKey) {
      const cached = cache.get(CacheNS.COLLEGE_FEED, cacheKey)
      if (cached) return { data: cached }
    }
    ...
    if (cacheKey) cache.set(CacheNS.COLLEGE_FEED, cacheKey, result, CacheTTL.COLLEGE_FEED)
    return { data: result }
  }
  ...
  if (path[0] === 'feed' && path[1] === 'house' && path.length === 3 && method === 'GET') {
    ...
    if (cacheKey) {
      const cached = cache.get(CacheNS.HOUSE_FEED, cacheKey)
      if (cached) return { data: cached }
    }
    ...
    if (cacheKey) cache.set(CacheNS.HOUSE_FEED, cacheKey, result, CacheTTL.HOUSE_FEED)
    return { data: result }
  }
  ...
  if (route === 'feed/reels' && method === 'GET') {
    ...
    const cacheKey = cursor ? null : `page1:limit${limit}`
    if (cacheKey) {
      const cached = cache.get(CacheNS.REELS_FEED, cacheKey)
      if (cached) return { data: cached }
    }
    ...
    if (cacheKey) cache.set(CacheNS.REELS_FEED, cacheKey, result, CacheTTL.REELS_FEED)
    return { data: result }
  }
