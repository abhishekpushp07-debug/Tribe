...
import { cache, CacheNS, CacheTTL, invalidateOnEvent } from '../cache.js'
...
import { startScheduledPublisher } from '../services/scheduled-publisher.js'
...
export async function handleContent(path, method, request, db) {
  ...
  // Lazy-init scheduled publisher worker
  startScheduledPublisher(db)
  ...
  if (route === 'content/posts' && method === 'POST') {
    ...
    const body = await request.json()
    const { caption, mediaIds, kind, syntheticDeclaration, visibility: requestedVisibility } = body
    const contentKind = kind || ContentKind.POST

    // Phase D: Extract new post sub-type fields
    const { poll, linkUrl, threadParentId } = body

    // Sprint 4: Carousel and scheduling
    const { carousel, status: postStatus, publishAt } = body
    ...
    if (carousel && contentKind === ContentKind.POST && mediaIds?.length > 1) {
      if (!Array.isArray(carousel.order) || carousel.order.length !== mediaIds.length) {
        return { error: 'carousel.order must be an array matching mediaIds length', code: ErrorCode.VALIDATION, status: 400 }
      }
      if (mediaIds.length > MAX_CAROUSEL_ITEMS) {
        return { error: `Carousel cannot exceed ${MAX_CAROUSEL_ITEMS} items`, code: ErrorCode.VALIDATION, status: 400 }
      }
      carouselData = {
        isCarousel: true,
        itemCount: mediaIds.length,
        order: carousel.order,
        coverIndex: carousel.coverIndex || 0,
        aspectRatio: carousel.aspectRatio || 'MIXED',
      }
    } else if (mediaIds?.length > 1) {
      carouselData = {
        isCarousel: true,
        itemCount: mediaIds.length,
        order: mediaIds.map((_, i) => i),
        coverIndex: 0,
        aspectRatio: 'MIXED',
      }
    }
    ...
    if (publishAt && contentKind === ContentKind.POST) {
      const publishDate = new Date(publishAt)
      if (isNaN(publishDate.getTime())) {
        return { error: 'Invalid publishAt date', code: ErrorCode.VALIDATION, status: 400 }
      }
      if (publishDate <= new Date()) {
        return { error: 'publishAt must be in the future', code: ErrorCode.VALIDATION, status: 400 }
      }
      const maxDate = new Date(Date.now() + MAX_SCHEDULED_AHEAD_DAYS * 86400000)
      if (publishDate > maxDate) {
        return { error: `Cannot schedule more than ${MAX_SCHEDULED_AHEAD_DAYS} days ahead`, code: ErrorCode.VALIDATION, status: 400 }
      }
      scheduledPublishAt = publishDate
      isDraft = true // Scheduled posts are drafts until published
    }
    ...
    // Honor the visibility sent by the frontend (audience selector)
    ...
    if (requestedVisibility && CONTENT_ALLOWED_VISIBILITIES.includes(requestedVisibility)) {
      ...
      return { error: `Invalid visibility. Must be one of: ${CONTENT_ALLOWED_VISIBILITIES.join(', ')}`, code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    // Drafts stay at stage 0 until published
    const moderationAction = moderationDecision?.action
    if (isDraft) {
      contentItem.intendedVisibility = visibility // Preserve for when draft is published
      contentItem.visibility = 'DRAFT'
      contentItem.distributionStage = 0
    } else if (moderationAction === 'HOLD' || moderationAction === 'ESCALATE' || moderationAction === 'REJECT') {
  ...
  // ========================
  // ========================
  // GET /content/drafts — List user's draft posts
  // ========================
  if (path[0] === 'content' && path[1] === 'drafts' && path.length === 2 && method === 'GET') {
    const user = await requireAuth(request, db)
    const url = new URL(request.url)
    const { limit, offset } = parsePagination(url)

    const drafts = await db.collection('content_items')
      .find({ authorId: user.id, isDraft: true, visibility: { $ne: Visibility.REMOVED } })
      .sort({ updatedAt: -1 })
      .skip(offset).limit(limit)
      .toArray()

    const total = await db.collection('content_items').countDocuments({ authorId: user.id, isDraft: true, visibility: { $ne: Visibility.REMOVED } })
    const enriched = await enrichPosts(db, drafts, user.id)

    return { data: { items: enriched, drafts: enriched, pagination: { total }, total } }
  }

  // ========================
  // GET /content/scheduled — List user's scheduled posts
  // ========================
  if (path[0] === 'content' && path[1] === 'scheduled' && path.length === 2 && method === 'GET') {
    const user = await requireAuth(request, db)
    const url = new URL(request.url)
    const { limit, offset } = parsePagination(url)

    const scheduled = await db.collection('content_items')
      .find({ authorId: user.id, publishAt: { $ne: null, $gt: new Date() }, isDraft: true, visibility: { $ne: Visibility.REMOVED } })
      .sort({ publishAt: 1 })
      .skip(offset).limit(limit)
      .toArray()

    const total = await db.collection('content_items').countDocuments({
      authorId: user.id, publishAt: { $ne: null, $gt: new Date() }, isDraft: true, visibility: { $ne: Visibility.REMOVED },
    })
    const enriched = await enrichPosts(db, scheduled, user.id)

    return { data: { items: enriched, scheduled: enriched, pagination: { total }, total } }
  }
  ...
  // ========================
  // POST /content/:id/publish — Publish a draft/scheduled post immediately
  // ========================
  if (path[0] === 'content' && path.length === 3 && path[2] === 'publish' && method === 'POST') {
    const user = await requireAuth(request, db)
    const contentId = path[1]
    const post = await db.collection('content_items').findOne({ id: contentId })
    ...
    if (post.authorId !== user.id) {
      return { error: 'Only author can publish their draft', code: ErrorCode.FORBIDDEN, status: 403 }
    }
    if (!post.isDraft) {
      return { error: 'Post is already published', code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    await db.collection('content_items').updateOne(
      { id: contentId },
      { $set: { isDraft: false, visibility: publishVisibility, publishAt: null, publishedAt: now, distributionStage: publishStage, updatedAt: now }, $unset: { intendedVisibility: '' } }
    )
    await db.collection('users').updateOne({ id: user.id }, { $inc: { postsCount: 1 } })
    await writeAudit(db, 'CONTENT_PUBLISHED', user.id, 'CONTENT', contentId, { wasScheduled: !!post.publishAt })

    const updated = await db.collection('content_items').findOne({ id: contentId })
    const enriched = await enrichPosts(db, [updated], user.id)
    return { data: { post: enriched[0], message: 'Post published' } }
  }

  // ========================
  // PATCH /content/:id/schedule — Update schedule for a draft
  // ========================
  if (path[0] === 'content' && path.length === 3 && path[2] === 'schedule' && method === 'PATCH') {
    const user = await requireAuth(request, db)
    const contentId = path[1]
    const body = await request.json()
    ...
    if (!post.isDraft) {
      return { error: 'Cannot reschedule a published post', code: ErrorCode.VALIDATION, status: 400 }
    }

    if (newPublishAt) {
      const publishDate = new Date(newPublishAt)
      if (isNaN(publishDate.getTime()) || publishDate <= new Date()) {
        return { error: 'publishAt must be a valid future date', code: ErrorCode.VALIDATION, status: 400 }
      }
      const maxDate = new Date(Date.now() + MAX_SCHEDULED_AHEAD_DAYS * 86400000)
      if (publishDate > maxDate) {
        return { error: `Cannot schedule more than ${MAX_SCHEDULED_AHEAD_DAYS} days ahead`, code: ErrorCode.VALIDATION, status: 400 }
      }
      await db.collection('content_items').updateOne({ id: contentId }, { $set: { publishAt: publishDate, updatedAt: new Date() } })
    } else {
      // Remove schedule (keep as draft without schedule)
      await db.collection('content_items').updateOne({ id: contentId }, { $set: { publishAt: null, updatedAt: new Date() } })
    }

    return { data: { message: newPublishAt ? 'Schedule updated' : 'Schedule removed', contentId } }
  }
