...
import { awardHousePoints } from './house-points.js'
...
export async function handleContent(path, method, request, db) {
  ...
  if (route === 'content/posts' && method === 'POST') {
    ...
    if (textToModerate.length > 0) {
      moderationResult = await moderateContent(textToModerate)
      if (moderationResult.flagged) {
        if (moderationResult.action === 'AUTO_REJECT') {
          visibility = 'HELD'  // Auto-held, requires admin review
        } else if (moderationResult.action === 'ESCALATE') {
          visibility = 'HELD'  // Escalated for review
        }
      }
    }
    ...
    const contentItem = {
      ...
      media,
      visibility,
      riskScore: moderationResult?.flagged ? (moderationResult.scores?.harassment || 0) : 0,
      policyReasons: moderationResult?.categories || [],
    ...
    // Award house points for content creation
    if (user.houseId) {
      const pointReason = contentKind === ContentKind.REEL ? 'REEL_CREATED' : contentKind === ContentKind.STORY ? 'STORY_CREATED' : 'POST_CREATED'
      const pointValue = contentKind === ContentKind.REEL ? 10 : contentKind === ContentKind.STORY ? 3 : 5
      await awardHousePoints(db, user.id, user.houseId, pointReason, pointValue, { contentId: contentItem.id })
    }
