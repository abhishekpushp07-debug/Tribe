...
export function startMediaCleanupWorker(db) {
  ...
  async function runCleanup() {
    try {
      ...
      // Find stale PENDING_UPLOAD records older than 24h
...
export async function generateVideoThumbnail(db, videoAsset) {
  ...
  try {
    ...
    await execAsync(
      `ffmpeg -i "${tmpInput}" -ss 00:00:00.500 -vframes 1 -q:v 2 -vf "scale=720:-2" "${tmpOutput}" -y`,
      { timeout: 15000 }
    )

    const thumbBuffer = await readFile(tmpOutput)
    if (thumbBuffer.length < 100) return null // Too small, extraction failed
...
export async function handleMediaCleanup(path, method, request, db) {
  ...
  if (path[0] === 'admin' && path[1] === 'media' && path[2] === 'cleanup' && method === 'POST') {
    ...
    const thresholdMs = body.thresholdMs || ORPHAN_THRESHOLD_MS
    ...
    const staleUploads = await db.collection('media_assets')
      ...
      .project({ _id: 0, id: 1, storagePath: 1, storageType: 1, createdAt: 1, ownerId: 1 })
      .limit(500)
      .toArray()
