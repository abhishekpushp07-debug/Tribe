  ...
  if (path_parts[0] === 'transcode' && path_parts[1] === 'queue' && path_parts.length === 2 && method === 'GET') {
    ...
    return {
      data: {
        ...
        total: stats.reduce((s, st) => s + st.count, 0),
      },
    }
  }

  // ========================
  // POST /transcode/:jobId/cancel — Cancel a pending/processing job
  // ========================
  if (path_parts[0] === 'transcode' && path_parts.length === 3 && path_parts[2] === 'cancel' && method === 'POST') {
    const user = await requireAuth(request, db)
    const jobId = path_parts[1]

    const job = await db.collection('transcode_jobs').findOne({ id: jobId })
    if (!job) return { error: 'Job not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (job.requesterId !== user.id && !['ADMIN', 'SUPER_ADMIN'].includes(user.role)) {
      return { error: 'Not your job', code: 'FORBIDDEN', status: 403 }
    }
    if (!['PENDING', 'PROCESSING'].includes(job.status)) {
      return { error: `Cannot cancel ${job.status} job`, code: ErrorCode.VALIDATION, status: 400 }
    }

    await db.collection('transcode_jobs').updateOne(
      { id: jobId },
      { $set: { status: 'CANCELLED', cancelledAt: new Date(), cancelledBy: user.id, updatedAt: new Date() } }
    )

    return { data: { message: 'Transcoding job cancelled', jobId } }
  }

  // ========================
  // POST /transcode/:jobId/retry — Retry a failed job
  // ========================
  if (path_parts[0] === 'transcode' && path_parts.length === 3 && path_parts[2] === 'retry' && method === 'POST') {
    const user = await requireAuth(request, db)
    const jobId = path_parts[1]
    ...
    if (job.status !== 'FAILED') {
      return { error: 'Only FAILED jobs can be retried', code: ErrorCode.VALIDATION, status: 400 }
    }
    if ((job.retryCount || 0) >= 3) {
      return { error: 'Max retry attempts reached (3)', code: 'RETRY_EXHAUSTED', status: 429 }
    }

    await db.collection('transcode_jobs').updateOne(
      { id: jobId },
      { $set: { status: 'PENDING', error: null, updatedAt: new Date() }, $inc: { retryCount: 1 } }
    )

    // Re-trigger processing
    const media = await db.collection('media_assets').findOne({ id: job.mediaId })
    if (media) {
      processTranscodeJob(db, jobId, media, job.qualities).catch(err => {
        console.error(`Transcode retry ${jobId} failed:`, err.message)
      })
    }

    return { data: { message: 'Transcoding job retried', jobId, retryCount: (job.retryCount || 0) + 1 }, status: 202 }
  }
...
async function processTranscodeJob(db, jobId, media, qualities) {
  ...
  const currentJob = await db.collection('transcode_jobs').findOne({ id: jobId })
  ...
  try {
    ...
    try {
      ...
      // Non-critical check - proceed even if we can't check
    } catch {}
    ...
    if (media.localPath && existsSync(media.localPath)) {
      ...
      for (let i = 0; i < applicableQualities.length; i++) {
        ...
        const jobState = await db.collection('transcode_jobs').findOne({ id: jobId }, { projection: { status: 1, _id: 0 } })
