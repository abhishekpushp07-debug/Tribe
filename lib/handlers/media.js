  ...
  if (path.join('/') === 'media/upload-complete' && method === 'POST') {
    ...
    if (asset.kind === 'VIDEO' || (asset.mimeType && asset.mimeType.startsWith('video/'))) {
      try {
        ...
        const thumbResult = await generateVideoThumbnail(db, updatedAsset)
        if (thumbResult) {
          thumbnailUrl = thumbResult.thumbnailUrl
          await db.collection('media_assets').updateOne(
            { id: mediaId },
            { $set: { thumbnailUrl: thumbResult.thumbnailUrl, thumbnailMediaId: thumbResult.thumbnailMediaId } }
          )
        }
      } catch (err) {
        const { default: logger } = await import('@/lib/logger')
        logger.warn('MEDIA', 'thumbnail_generation_failed', { error: err.message, mediaId })
      }
    }
