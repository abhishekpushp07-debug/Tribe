...
export async function handleMedia(path, method, request, db) {
  ...
  if ((path.join('/') === 'media/upload-complete' || path.join('/') === 'media/complete') && method === 'POST') {
    ...
    // Generate thumbnail for video uploads (ASYNC — non-blocking)
    // Client should poll GET /media/upload-status/:id to check thumbnailStatus
    let thumbnailStatus = 'NONE'
    if (asset.kind === 'VIDEO' || (asset.mimeType && asset.mimeType.startsWith('video/'))) {
      thumbnailStatus = 'PENDING'
      // Fire-and-forget: thumbnail generation runs in background
      ;(async () => {
        try {
          const { generateVideoThumbnail } = await import('./media-cleanup.js')
          const updatedAsset = { ...asset, ...updates }
          await generateVideoThumbnail(db, updatedAsset)
        } catch (err) {
          const { default: logger } = await import('@/lib/logger')
          logger.warn('MEDIA', 'async_thumbnail_failed', { error: err.message, mediaId })
        }
      })()
    }
    ...
    return {
      data: {
        ...
        url: asset.publicUrl || `/api/media/${asset.id}`,
        publicUrl: asset.publicUrl,
        thumbnailUrl: null, // Will be available when thumbnailStatus=READY (poll upload-status)
        thumbnailStatus,
        type: asset.type,
        kind: asset.kind,
        mimeType: asset.mimeType,
