...
export async function handleDiscovery(path, method, request, db) {
  ...
  if (route === 'houses' && method === 'GET') {
    const cached = cache.get(CacheNS.HOUSES_LIST, 'all')
    ...
    cache.set(CacheNS.HOUSES_LIST, 'all', result, CacheTTL.HOUSES_LIST)
    return { data: result }
  }
  ...
  if (route === 'houses/leaderboard' && method === 'GET') {
    const cached = cache.get(CacheNS.HOUSE_LEADERBOARD, 'all')
    if (cached) return { data: cached }
    ...
    cache.set(CacheNS.HOUSE_LEADERBOARD, 'all', result, CacheTTL.HOUSE_LEADERBOARD)
    return { data: result }
  }
