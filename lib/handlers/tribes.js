...
export async function handleTribes(path, method, request, db) {
  ...
  if (path[0] === 'tribes' && path.length === 3 && path[2] === 'join' && method === 'POST') {
    ...
    await db.collection('tribes').updateOne({ id: tribe.id }, { $inc: { membersCount: 1 } })

    return { data: { message: `Joined tribe ${tribe.tribeName || tribe.tribeCode}`, tribeId: tribe.id }, status: 201 }
  }
  ...
  if (path[0] === 'tribes' && path.length === 3 && path[2] === 'leave' && method === 'POST') {
    ...
    await db.collection('user_tribe_memberships').updateOne({ id: membership.id }, { $set: { status: 'LEFT', leftAt: new Date() } })
    await db.collection('tribes').updateOne({ id: tribe.id }, { $inc: { membersCount: -1 } })
  ...
  // ========================
  // POST /tribes/:id/cheer — Cheer for your tribe
  // ========================
  if (path[0] === 'tribes' && path.length === 3 && path[2] === 'cheer' && method === 'POST') {
    ...
    await db.collection('tribe_cheers').insertOne({
      id: uuidv4(), userId: user.id, tribeId: tribe.id, createdAt: new Date(),
    })
    await db.collection('tribes').updateOne({ id: tribe.id }, { $inc: { cheerCount: 1, totalSalutes: 1 } })

    return { data: { message: `Cheered for ${tribe.tribeName || tribe.tribeCode}!`, cheerCount: (tribe.cheerCount || 0) + 1 } }
  }
