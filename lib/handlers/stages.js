...
export async function handleResources(path, method, request, db) {
  ...
  if (path[0] === 'resources' && path.length === 2 && path[1] !== 'search' && method === 'GET') {
    ...
    if (result.status === ResourceStatus.REMOVED) return { error: 'Resource has been removed', code: 'GONE', status: 410 }
  ...
  if (path[0] === 'resources' && path.length === 2 && method === 'PATCH') {
    ...
    if (updates.title || updates.description) {
      ...
      try {
        ...
        if (err.details?.code === 'CONTENT_REJECTED_BY_MODERATION') {
          return { error: 'Updated content rejected by moderation', code: 'CONTENT_REJECTED', status: 422 }
        }
        throw err
      }
    }
  ...
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'report' && method === 'POST') {
    ...
    const resource = await db.collection('resources').findOne({ id: resourceId })
    if (!resource) return { error: 'Resource not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (resource.status === ResourceStatus.REMOVED) return { error: 'Resource already removed', code: 'GONE', status: 410 }
  ...
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'download' && method === 'POST') {
    ...
    const resource = await db.collection('resources').findOne({ id: resourceId }, { projection: { _id: 0 } })
    if (!resource) return { error: 'Resource not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (resource.status === ResourceStatus.REMOVED) return { error: 'Resource removed', code: 'GONE', status: 410 }
