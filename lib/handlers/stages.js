...
import { requireAuth, requireRole, authenticate, writeAudit, sanitizeUser, parsePagination } from '../auth-utils.js'
import { ErrorCode, Visibility, AppealStatus, ModerationAction, ClaimStatus, ClaimConfig, ResourceKind, ResourceStatus, ResourceConfig } from '../constants.js'
...
export async function handleCollegeClaims(path, method, request, db) {
  ...
  if (route === 'me/college-claims' && method === 'GET') {
    ...
    return { data: { items: enriched, claims: enriched, count: claims.length } }
  }
...
// ═══════════════════════════════════════════════════
// STAGE 5 — Notes / PYQs Library (World-Class)
//
// 12 endpoints, Redis-cached, text-indexed, vote system,
// admin moderation, college-membership guard, explainPlan-safe.
// ═══════════════════════════════════════════════════

async function invalidateResource(resourceId) {
  await invalidateOnEvent('RESOURCE_CHANGED', { resourceId })
}

/**
 * Compute vote trust weight for a user.
 * Low-trust accounts (< 7 days old OR active strikes) get 0.5x weight.
 */
async function getVoterTrustWeight(db, user) {
  const now = new Date()
  const accountAgeDays = (now - new Date(user.createdAt)) / (1000 * 60 * 60 * 24)

  if (accountAgeDays < ResourceConfig.LOW_TRUST_ACCOUNT_AGE_DAYS) {
    return ResourceConfig.LOW_TRUST_VOTE_WEIGHT
  }
  ...
  if (activeStrikes > 0) return ResourceConfig.LOW_TRUST_VOTE_WEIGHT
  return 1.0
}

/**
 * Recompute vote counters from source of truth (resource_votes collection).
 * Returns { voteScore, trustedVoteScore, voteCount }.
 */
async function recomputeVoteCounters(db, resourceId) {
  const votes = await db.collection('resource_votes')
    .find({ resourceId })
    .toArray()
  ...
  if (voterIds.length > 0) {
    const now = new Date()
    const voters = await db.collection('users').find({ id: { $in: voterIds } }).toArray()
    const activeStrikes = await db.collection('strikes').find({
      userId: { $in: voterIds },
      reversed: { $ne: true },
      expiresAt: { $gt: now },
    }).toArray()
    const struckIds = new Set(activeStrikes.map(s => s.userId))

    for (const u of voters) {
      const ageDays = (now - new Date(u.createdAt)) / (1000 * 60 * 60 * 24)
      const isLowTrust = ageDays < ResourceConfig.LOW_TRUST_ACCOUNT_AGE_DAYS || struckIds.has(u.id)
      voterMap[u.id] = isLowTrust ? ResourceConfig.LOW_TRUST_VOTE_WEIGHT : 1.0
    }
  }

  for (const v of votes) {
    const delta = v.vote === 'UP' ? 1 : -1
    const weight = voterMap[v.voterId] || 1.0
    rawScore += delta
    trustedScore += delta * weight
  }

  return {
    voteScore: rawScore,
    trustedVoteScore: Math.round(trustedScore * 10) / 10,
    voteCount: votes.length,
  }
}

/**
 * Recompute download count from source of truth (resource_downloads collection).
 */
async function recomputeDownloadCount(db, resourceId) {
  return db.collection('resource_downloads').countDocuments({ resourceId })
}

function validateResourceInput(body) {
  const { kind, collegeId, title, subject, semester, year, description } = body
  const errors = []

  if (!kind || !ResourceConfig.VALID_KINDS.includes(kind)) {
    errors.push(`kind must be one of: ${ResourceConfig.VALID_KINDS.join(', ')}`)
  }
  if (!collegeId) errors.push('collegeId is required')
  if (!title || title.length < ResourceConfig.MIN_TITLE_LENGTH) {
    errors.push(`title must be at least ${ResourceConfig.MIN_TITLE_LENGTH} characters`)
  }
  if (title && title.length > ResourceConfig.MAX_TITLE_LENGTH) {
    errors.push(`title must be at most ${ResourceConfig.MAX_TITLE_LENGTH} characters`)
  }
  if (description && description.length > ResourceConfig.MAX_DESCRIPTION_LENGTH) {
    errors.push(`description must be at most ${ResourceConfig.MAX_DESCRIPTION_LENGTH} characters`)
  }
  if (semester !== undefined && semester !== null) {
    const s = Number(semester)
    if (isNaN(s) || s < 1 || s > 12) errors.push('semester must be between 1 and 12')
  }
  if (year !== undefined && year !== null) {
    const y = Number(year)
    if (isNaN(y) || y < ResourceConfig.MIN_YEAR || y > ResourceConfig.MAX_YEAR) {
      errors.push(`year must be between ${ResourceConfig.MIN_YEAR} and ${ResourceConfig.MAX_YEAR}`)
    }
  }
  if (kind === 'PYQ' && !subject) errors.push('subject is required for PYQ resources')
...
export async function handleResources(path, method, request, db) {
  ...
  // Guard: auth + adult + college membership
  // ──────────────────────────────────────────────
  if (route === 'resources' && method === 'POST') {
    ...
    // Upload rate limit: max 10 resources per user per hour
    const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000)
    const recentUploads = await db.collection('resources').countDocuments({
      uploaderId: user.id,
      createdAt: { $gt: oneHourAgo },
    })
    if (recentUploads >= ResourceConfig.HOURLY_UPLOAD_RATE_LIMIT) {
      return {
        error: `Upload rate limit exceeded (max ${ResourceConfig.HOURLY_UPLOAD_RATE_LIMIT} per hour)`,
    ...
    const body = await request.json()
    const validationErrors = validateResourceInput(body)
    if (validationErrors.length > 0) {
      return { error: validationErrors.join('; '), code: ErrorCode.VALIDATION, status: 400 }
    }

    const { kind, collegeId, branch, subject, semester, year, title, description, fileAssetId } = body

    // College membership guard: user must belong to this college
    if (user.collegeId !== collegeId && !['MODERATOR', 'ADMIN', 'SUPER_ADMIN'].includes(user.role)) {
      return { error: 'You can only upload resources to your own college', code: ErrorCode.FORBIDDEN, status: 403 }
    }

    // Verify college exists
    const college = await db.collection('colleges').findOne({ id: collegeId })
    if (!college) {
      return { error: 'College not found', code: ErrorCode.NOT_FOUND, status: 404 }
    }
    ...
    try {
      ...
      if (err.details?.code === 'CONTENT_REJECTED_BY_MODERATION') {
        return { error: 'Resource content rejected by moderation', code: ErrorCode.CONTENT_REJECTED, status: 422 }
      }
      throw err
    }
    ...
    const resource = {
      ...
      kind,
      uploaderId: user.id,
      uploaderCollegeId: user.collegeId || collegeId,
      collegeId,
      collegeName: college.officialName || college.name || null,
      branch: branch || null,
      subject: subject || null,
      semester: semester ? Number(semester) : null,
      year: year ? Number(year) : null,
      title: title.slice(0, ResourceConfig.MAX_TITLE_LENGTH),
      description: (description || '').slice(0, ResourceConfig.MAX_DESCRIPTION_LENGTH),
    ...
    await writeAudit(db, 'RESOURCE_CREATED', user.id, 'RESOURCE', resource.id, { kind, collegeId, subject })
    await invalidateResource(resource.id)
  ...
  // ──────────────────────────────────────────────
  // GET /resources/search — Faceted search + text search, Redis-cached
  // Public (no auth required)
  // Filters: collegeId, branch, subject, semester, kind, year, q, sort
  // ──────────────────────────────────────────────
  if (route === 'resources/search' && method === 'GET') {
    ...
    const cacheKey = JSON.stringify({ collegeId, branch, subject, semester, kind, year, q, sort, uploaderId, cursor, limit })

    const result = await cache.getOrCompute(CacheNS.RESOURCE_SEARCH, cacheKey, async () => {
      const query = { status: ResourceStatus.PUBLIC }

      if (collegeId) query.collegeId = collegeId
      if (branch) query.branch = { $regex: `^${branch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, $options: 'i' }
      if (subject) query.subject = { $regex: subject.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' }
      if (semester) query.semester = Number(semester)
      if (kind) {
        if (kind.includes(',')) {
          query.kind = { $in: kind.split(',').filter(k => ResourceConfig.VALID_KINDS.includes(k)) }
        } else {
          query.kind = kind
        }
      }
      ...
      // Full-text search via $text if available, fallback to regex
      if (q) {
        query.$or = [
      ...
      // Sorting — popular uses trustedVoteScore for anti-gaming
      let sortSpec = { createdAt: -1 }
      if (sort === 'popular') sortSpec = { trustedVoteScore: -1, createdAt: -1 }
      else if (sort === 'most_downloaded') sortSpec = { downloadCount: -1, createdAt: -1 }

      // Cursor-based pagination (use id for uniqueness, not createdAt alone)
      if (cursor) {
        const cursorDoc = await db.collection('resources').findOne({ id: cursor }, { projection: { createdAt: 1, trustedVoteScore: 1, downloadCount: 1 } })
        if (cursorDoc) {
          if (sort === 'popular') {
            query.$or = query.$or || []
            // If $or already has text search, wrap in $and
            const cursorFilter = {
              $or: [
                { trustedVoteScore: { $lt: cursorDoc.trustedVoteScore || 0 } },
                { trustedVoteScore: cursorDoc.trustedVoteScore || 0, createdAt: { $lt: cursorDoc.createdAt } },
                { trustedVoteScore: cursorDoc.trustedVoteScore || 0, createdAt: cursorDoc.createdAt, id: { $lt: cursor } },
              ]
            }
            if (query.$or) {
              const textFilter = query.$or
              delete query.$or
              query.$and = [{ $or: textFilter }, cursorFilter]
            } else {
            ...
            const cursorFilter = {
              $or: [
                { downloadCount: { $lt: cursorDoc.downloadCount } },
                { downloadCount: cursorDoc.downloadCount, createdAt: { $lt: cursorDoc.createdAt } },
              ]
            }
            if (query.$or) {
              const textFilter = query.$or
            ...
            // Default: recent
            const cursorFilter = { createdAt: { $lt: cursorDoc.createdAt } }
            if (query.$or) {
              const textFilter = query.$or
              delete query.$or
              query.$and = [{ $or: textFilter }, cursorFilter]
      ...
      const users = uploaderIds.length > 0
        ? await db.collection('users').find({ id: { $in: uploaderIds } }, { projection: { _id: 0, pinHash: 0, pinSalt: 0 } }).toArray()
        : []
      const userMap = Object.fromEntries(users.map(u => [u.id, sanitizeUser(u)]))
      const enriched = items.map(r => ({ ...r, uploader: userMap[r.uploaderId] || null }))

      // Facet counts for the current college (if filtered)
      let facets = null
      if (collegeId) {
        const baseQuery = { status: ResourceStatus.PUBLIC, collegeId }
        const [kindCounts, semesterCounts, branchCounts] = await Promise.all([
          db.collection('resources').aggregate([
            { $match: baseQuery },
            { $group: { _id: '$kind', count: { $sum: 1 } } },
          ]).toArray(),
          db.collection('resources').aggregate([
            { $match: { ...baseQuery, semester: { $ne: null } } },
            { $group: { _id: '$semester', count: { $sum: 1 } } },
            { $sort: { _id: 1 } },
          ]).toArray(),
          db.collection('resources').aggregate([
            { $match: { ...baseQuery, branch: { $ne: null } } },
        ...
        facets = {
          kinds: Object.fromEntries(kindCounts.map(k => [k._id, k.count])),
          semesters: Object.fromEntries(semesterCounts.map(s => [String(s._id), s.count])),
          branches: Object.fromEntries(branchCounts.map(b => [b._id, b.count])),
        }
      }
    ...
    }, CacheTTL.RESOURCE_SEARCH)

    return { data: result }
  }

  // ──────────────────────────────────────────────
  // GET /resources/:id — Detail view (cached)
  // Public for PUBLIC resources. HELD requires owner/admin. REMOVED returns 410.
  // ──────────────────────────────────────────────
  if (path[0] === 'resources' && path.length === 2 && path[1] !== 'search' && method === 'GET') {
    const resourceId = path[1]

    const result = await cache.getOrCompute(CacheNS.RESOURCE_DETAIL, resourceId, async () => {
      const resource = await db.collection('resources').findOne({ id: resourceId }, { projection: { _id: 0 } })
      if (!resource) return null

      const [uploader, tags, college] = await Promise.all([
        db.collection('users').findOne({ id: resource.uploaderId }),
        db.collection('authenticity_tags').find({ targetType: 'RESOURCE', targetId: resourceId }, { projection: { _id: 0 } }).toArray(),
        db.collection('colleges').findOne({ id: resource.collegeId }, { projection: { _id: 0, officialName: 1, name: 1, id: 1, city: 1 } }),
      ])
    ...
    }, CacheTTL.RESOURCE_DETAIL)

    if (!result) return { error: 'Resource not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (result.status === ResourceStatus.REMOVED) return { error: 'Resource has been removed', code: ErrorCode.GONE, status: 410 }

    // HELD visibility tightening: only owner or admin can view HELD resources
    if (result.status === ResourceStatus.HELD) {
      const viewer = await authenticate(request, db)
      if (!viewer) {
        return { error: 'Resource is under review', code: ErrorCode.FORBIDDEN, status: 403 }
      }
      const isOwner = viewer.id === result.uploaderId
      const isPrivileged = ['MODERATOR', 'ADMIN', 'SUPER_ADMIN'].includes(viewer.role)
  ...
  // ──────────────────────────────────────────────
  if (path[0] === 'resources' && path.length === 2 && method === 'PATCH') {
    const user = await requireAuth(request, db)
    const resourceId = path[1]
    ...
    if (resource.uploaderId !== user.id && !['MODERATOR', 'ADMIN', 'SUPER_ADMIN'].includes(user.role)) {
      return { error: 'Only the uploader can edit this resource', code: ErrorCode.FORBIDDEN, status: 403 }
    }
    if (resource.status === ResourceStatus.REMOVED) {
      return { error: 'Cannot edit a removed resource', code: ErrorCode.FORBIDDEN, status: 403 }
    }

    const body = await request.json()
    const updates = {}
    const allowed = ['title', 'description', 'branch', 'subject', 'semester', 'year', 'kind', 'fileAssetId']

    for (const field of allowed) {
      if (body[field] !== undefined) {
        if (field === 'title') {
          if (body.title.length < ResourceConfig.MIN_TITLE_LENGTH) return { error: 'title too short', code: ErrorCode.VALIDATION, status: 400 }
          updates.title = body.title.slice(0, ResourceConfig.MAX_TITLE_LENGTH)
        } else if (field === 'description') {
          updates.description = (body.description || '').slice(0, ResourceConfig.MAX_DESCRIPTION_LENGTH)
        } else if (field === 'semester') {
          const s = Number(body.semester)
          if (isNaN(s) || s < 1 || s > 12) return { error: 'semester must be 1-12', code: ErrorCode.VALIDATION, status: 400 }
          updates.semester = s
        } else if (field === 'year') {
          const y = Number(body.year)
          if (isNaN(y) || y < ResourceConfig.MIN_YEAR || y > ResourceConfig.MAX_YEAR) return { error: 'Invalid year', code: ErrorCode.VALIDATION, status: 400 }
          updates.year = y
        } else if (field === 'kind') {
          if (!ResourceConfig.VALID_KINDS.includes(body.kind)) return { error: 'Invalid kind', code: ErrorCode.VALIDATION, status: 400 }
    ...
    // Moderate title/description changes
    if (updates.title || updates.description) {
      const textToModerate = [updates.title || resource.title, updates.description || resource.description].filter(Boolean).join(' ')
      try {
        await moderateCreateContent(db, {
          entityType: 'resource',
          actorUserId: user.id,
          text: textToModerate,
          metadata: { route: 'PATCH /resources/:id', resourceId },
        })
      } catch (err) {
        if (err.details?.code === 'CONTENT_REJECTED_BY_MODERATION') {
          return { error: 'Updated content rejected by moderation', code: ErrorCode.CONTENT_REJECTED, status: 422 }
        }
        throw err
      }
    }
    ...
    await db.collection('resources').updateOne({ id: resourceId }, { $set: updates })
    await writeAudit(db, 'RESOURCE_UPDATED', user.id, 'RESOURCE', resourceId, { fields: Object.keys(updates) })
    await invalidateResource(resourceId)

    const updated = await db.collection('resources').findOne({ id: resourceId }, { projection: { _id: 0 } })
    return { data: { resource: updated } }
  }

  // ──────────────────────────────────────────────
  // DELETE /resources/:id — Soft remove (owner or mod)
  // ──────────────────────────────────────────────
  if (path[0] === 'resources' && path.length === 2 && method === 'DELETE') {
    ...
    await db.collection('resources').updateOne(
      { id: resourceId },
      { $set: { status: ResourceStatus.REMOVED, removedBy: user.id, removedAt: new Date(), updatedAt: new Date() } }
    )
    await writeAudit(db, 'RESOURCE_REMOVED', user.id, 'RESOURCE', resourceId, { removedByRole: user.role })
    await invalidateResource(resourceId)

    return { data: { message: 'Resource removed', resourceId } }
  }

  // ──────────────────────────────────────────────
  // POST /resources/:id/report — Report resource (auto-hold at 3+)
  // ──────────────────────────────────────────────
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'report' && method === 'POST') {
    ...
    const resource = await db.collection('resources').findOne({ id: resourceId })
    if (!resource) return { error: 'Resource not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (resource.status === ResourceStatus.REMOVED) return { error: 'Resource already removed', code: ErrorCode.GONE, status: 410 }

    // Prevent duplicate reports (check + atomic insert with race handling)
    const existingReport = await db.collection('reports').findOne({ reporterId: user.id, targetType: 'RESOURCE', targetId: resourceId })
    if (existingReport) return { error: 'You have already reported this resource', code: ErrorCode.CONFLICT, status: 409 }
    ...
    // Atomic dedup: use updateOne with upsert to prevent TOCTOU race
    const upsertResult = await db.collection('reports').updateOne(
      { reporterId: user.id, targetType: 'RESOURCE', targetId: resourceId },
      { $setOnInsert: report },
      { upsert: true }
    )

    if (upsertResult.upsertedCount === 0) {
      // Document already existed — concurrent duplicate
      return { error: 'You have already reported this resource', code: ErrorCode.CONFLICT, status: 409 }
    }

    // Atomically increment reportCount and auto-hold
    const updated = await db.collection('resources').findOneAndUpdate(
      { id: resourceId },
      { $inc: { reportCount: 1 }, $set: { updatedAt: new Date() } },
      { returnDocument: 'after' }
    )

    if (updated && updated.reportCount >= ResourceConfig.AUTO_HOLD_REPORT_THRESHOLD && updated.status === ResourceStatus.PUBLIC) {
      await db.collection('resources').updateOne(
        { id: resourceId },
        { $set: { status: ResourceStatus.HELD, heldAt: new Date(), heldReason: 'AUTO_REPORT_THRESHOLD', updatedAt: new Date() } }
      )
      await writeAudit(db, 'RESOURCE_AUTO_HELD', 'SYSTEM', 'RESOURCE', resourceId, { reportCount: updated.reportCount })
    }

    await writeAudit(db, 'RESOURCE_REPORTED', user.id, 'RESOURCE', resourceId, { reasonCode: report.reasonCode })
    await invalidateResource(resourceId)
  ...
  // ──────────────────────────────────────────────
  // POST /resources/:id/vote — Upvote/downvote (helpfulness)
  // Trust-weighted: low-trust accounts get 0.5x weight
  // ──────────────────────────────────────────────
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'vote' && method === 'POST') {
    const user = await requireAuth(request, db)
    const resourceId = path[1]

    const resource = await db.collection('resources').findOne({ id: resourceId })
    if (!resource) return { error: 'Resource not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (resource.status !== ResourceStatus.PUBLIC) return { error: 'Cannot vote on this resource', code: ErrorCode.FORBIDDEN, status: 403 }
    if (resource.uploaderId === user.id) return { error: 'Cannot vote on your own resource', code: ErrorCode.FORBIDDEN, status: 403 }

    const body = await request.json()
    const vote = body.vote
    if (!ResourceConfig.VALID_VOTE_TYPES.includes(vote)) {
      return { error: 'vote must be UP or DOWN', code: ErrorCode.VALIDATION, status: 400 }
    }

    const trustWeight = await getVoterTrustWeight(db, user)
    const existing = await db.collection('resource_votes').findOne({ resourceId, voterId: user.id })

    if (existing) {
      if (existing.vote === vote) {
        return { error: 'You already voted this way', code: ErrorCode.CONFLICT, status: 409 }
      }
      // Switch vote: update record, then recompute from source
      await db.collection('resource_votes').updateOne(
        { resourceId, voterId: user.id },
        { $set: { vote, trustWeight, updatedAt: new Date() } }
      )
    } else {
      // New vote
      await db.collection('resource_votes').insertOne({
        id: uuidv4(),
        resourceId,
        voterId: user.id,
        vote,
    ...
    // Recompute from source of truth (prevents drift)
    const counters = await recomputeVoteCounters(db, resourceId)
    await db.collection('resources').updateOne(
      { id: resourceId },
    ...
    return { data: { vote, ...counters, trustWeight } }
  }

  // ──────────────────────────────────────────────
  // DELETE /resources/:id/vote — Remove vote (recomputes from source)
  // ──────────────────────────────────────────────
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'vote' && method === 'DELETE') {
    const user = await requireAuth(request, db)
    const resourceId = path[1]

    const existing = await db.collection('resource_votes').findOne({ resourceId, voterId: user.id })
    if (!existing) return { error: 'No vote to remove', code: ErrorCode.NOT_FOUND, status: 404 }

    await db.collection('resource_votes').deleteOne({ resourceId, voterId: user.id })
    ...
    return { data: { message: 'Vote removed', ...counters } }
  }

  // ──────────────────────────────────────────────
  // POST /resources/:id/download — Track download (auth required)
  // Rate limited: max 50 unique resource downloads per user per 24h.
  // Per-user per-resource 24h dedup.
  // Returns fileAssetId for client to fetch from storage.
  // ──────────────────────────────────────────────
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'download' && method === 'POST') {
    const user = await requireAuth(request, db)
    const resourceId = path[1]

    const resource = await db.collection('resources').findOne({ id: resourceId }, { projection: { _id: 0 } })
    if (!resource) return { error: 'Resource not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (resource.status === ResourceStatus.REMOVED) return { error: 'Resource removed', code: ErrorCode.GONE, status: 410 }

    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000)

    // Rate limit: count distinct resources downloaded in last 24h
    const dailyDownloads = await db.collection('resource_downloads').countDocuments({
      userId: user.id,
      createdAt: { $gt: oneDayAgo },
    })
    if (dailyDownloads >= ResourceConfig.DAILY_DOWNLOAD_RATE_LIMIT) {
      return {
        error: `Download rate limit exceeded (max ${ResourceConfig.DAILY_DOWNLOAD_RATE_LIMIT} per 24h)`,
        code: ErrorCode.RATE_LIMITED,
        status: 429,
      }
    }

    // Per-resource dedup: one per user per resource per 24h
    const recentDownload = await db.collection('resource_downloads').findOne({
      resourceId,
      userId: user.id,
      createdAt: { $gt: oneDayAgo },
    })
    ...
    if (!recentDownload) {
      await db.collection('resource_downloads').insertOne({
        resourceId,
        userId: user.id,
        createdAt: new Date(),
      })
      await db.collection('resources').updateOne({ id: resourceId }, { $inc: { downloadCount: 1 } })
    }
    ...
    return {
      data: {
        ...
        downloadCount: resource.downloadCount + (recentDownload ? 0 : 1),
      },
    }
  }
  ...
  // ──────────────────────────────────────────────
  if (route === 'me/resources' && method === 'GET') {
    const user = await requireAuth(request, db)
    const url = new URL(request.url)
    const { limit, cursor } = parsePagination(url)
    const status = url.searchParams.get('status')

    const query = { uploaderId: user.id }
    if (status && [ResourceStatus.PUBLIC, ResourceStatus.HELD, ResourceStatus.REMOVED].includes(status)) {
      query.status = status
    }
    if (cursor) {
      const cursorDoc = await db.collection('resources').findOne({ id: cursor }, { projection: { createdAt: 1 } })
      if (cursorDoc) query.createdAt = { $lt: cursorDoc.createdAt }
    }
  ...
  // ──────────────────────────────────────────────
  if (route === 'admin/resources' && method === 'GET') {
    const user = await requireAuth(request, db)
    requireRole(user, 'MODERATOR', 'ADMIN', 'SUPER_ADMIN')

    const url = new URL(request.url)
    const { limit, cursor } = parsePagination(url)
    const status = url.searchParams.get('status') || ResourceStatus.HELD
    const collegeId = url.searchParams.get('collegeId')
    ...
    // Queue stats
    const [totalHeld, totalPublic, totalRemoved] = await Promise.all([
      db.collection('resources').countDocuments({ status: ResourceStatus.HELD }),
      db.collection('resources').countDocuments({ status: ResourceStatus.PUBLIC }),
      db.collection('resources').countDocuments({ status: ResourceStatus.REMOVED }),
    ])
    ...
    return {
      data: {
        ...
        nextCursor: hasMore ? items[items.length - 1].id : null,
        stats: { held: totalHeld, public: totalPublic, removed: totalRemoved },
      },
    }
  }
  ...
  // ──────────────────────────────────────────────
  if (path[0] === 'admin' && path[1] === 'resources' && path.length === 4 && path[3] === 'moderate' && method === 'PATCH') {
    const user = await requireAuth(request, db)
    requireRole(user, 'MODERATOR', 'ADMIN', 'SUPER_ADMIN')
    ...
    const body = await request.json()
    const action = body.action // 'APPROVE', 'HOLD', 'REMOVE'
    const reason = body.reason || ''

    const validActions = { APPROVE: ResourceStatus.PUBLIC, HOLD: ResourceStatus.HELD, REMOVE: ResourceStatus.REMOVED }
    if (!validActions[action]) {
      return { error: 'action must be APPROVE, HOLD, or REMOVE', code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    await writeAudit(db, 'RESOURCE_MODERATED', user.id, 'RESOURCE', resourceId, {
      action,
    ...
    const actionPast = { APPROVE: 'approved', HOLD: 'held', REMOVE: 'removed' }
    return { data: { message: `Resource ${actionPast[action]}`, resourceId, status: newStatus } }
  }

  // ──────────────────────────────────────────────
  // POST /admin/resources/:id/recompute-counters — Recompute from source
  // Fixes any drift in voteScore/voteCount/downloadCount.
  // ──────────────────────────────────────────────
  if (path[0] === 'admin' && path[1] === 'resources' && path.length === 4 && path[3] === 'recompute-counters' && method === 'POST') {
    const user = await requireAuth(request, db)
    ...
    const [voteCounters, downloadCount] = await Promise.all([
      recomputeVoteCounters(db, resourceId),
      recomputeDownloadCount(db, resourceId),
    ])

    const before = {
      voteScore: resource.voteScore,
      trustedVoteScore: resource.trustedVoteScore,
    ...
    await writeAudit(db, 'RESOURCE_COUNTERS_RECOMPUTED', user.id, 'RESOURCE', resourceId, { before, after })
    await invalidateResource(resourceId)

    return { data: { message: 'Counters recomputed from source of truth', resourceId, before, after } }
  }

  // ──────────────────────────────────────────────
  // POST /admin/resources/reconcile — Bulk reconciliation
  // Recomputes counters for all resources with drift detected.
  // ──────────────────────────────────────────────
  if (route === 'admin/resources/reconcile' && method === 'POST') {
    const user = await requireAuth(request, db)
    requireRole(user, 'ADMIN', 'SUPER_ADMIN')

    const resources = await db.collection('resources')
      .find({ status: { $ne: ResourceStatus.REMOVED } })
      .project({ id: 1, voteScore: 1, voteCount: 1, downloadCount: 1, trustedVoteScore: 1, _id: 0 })
      .toArray()
    ...
    for (const r of resources) {
      const [voteCounters, downloadCount] = await Promise.all([
        recomputeVoteCounters(db, r.id),
        recomputeDownloadCount(db, r.id),
      ])

      const hasDrift =
        r.voteScore !== voteCounters.voteScore ||
        r.voteCount !== voteCounters.voteCount ||
        r.downloadCount !== downloadCount ||
        (r.trustedVoteScore || 0) !== voteCounters.trustedVoteScore

      if (hasDrift) {
        await db.collection('resources').updateOne(
          { id: r.id },
          { $set: { ...voteCounters, downloadCount, updatedAt: new Date() } }
        )
        drifts.push({
          resourceId: r.id,
          before: { voteScore: r.voteScore, voteCount: r.voteCount, downloadCount: r.downloadCount, trustedVoteScore: r.trustedVoteScore || 0 },
          after: { ...voteCounters, downloadCount },
        })
        fixed++
      }
    }
    ...
    if (fixed > 0) {
      await invalidateOnEvent('RESOURCE_CHANGED', {})
      await writeAudit(db, 'RESOURCE_BULK_RECONCILIATION', user.id, 'SYSTEM', 'RESOURCES', { checked: resources.length, fixed, drifts })
    }

    return { data: { message: 'Reconciliation complete', checked: resources.length, fixed, drifts } }
  }
...
export async function handleEvents(path, method, request, db) {
  ...
  if (route === 'events' && method === 'POST') {
    ...
    try {
      ...
      if (err.details?.code === 'CONTENT_REJECTED_BY_MODERATION') {
        return { error: 'Event content rejected by moderation', code: ErrorCode.CONTENT_REJECTED, status: 422 }
      }
      throw err
    }
...
export async function handleAuthenticityTags(path, method, request, db) {
  ...
  if (path[0] === 'authenticity' && path[1] === 'tags' && path.length === 4 && method === 'GET') {
    ...
    return { data: { items: tags, tags, count: tags.length } }
  }
