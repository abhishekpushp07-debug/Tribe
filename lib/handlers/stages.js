...
export async function handleResources(path, method, request, db) {
  ...
  if (route === 'resources/search' && method === 'GET') {
    ...
    const result = await cache.getOrCompute(CacheNS.RESOURCE_SEARCH, cacheKey, async () => {
      ...
      if (sort === 'popular') sortSpec = { voteScore: -1, createdAt: -1 }
      ...
      if (cursor) {
        const cursorDoc = await db.collection('resources').findOne({ id: cursor }, { projection: { createdAt: 1, voteScore: 1, downloadCount: 1 } })
        if (cursorDoc) {
          if (sort === 'popular') {
            query.$or = query.$or || []
            ...
            const cursorFilter = {
              $or: [
                { voteScore: { $lt: cursorDoc.voteScore } },
                { voteScore: cursorDoc.voteScore, createdAt: { $lt: cursorDoc.createdAt } },
                { voteScore: cursorDoc.voteScore, createdAt: cursorDoc.createdAt, id: { $lt: cursor } },
              ]
            }
            if (query.$or) {
  ...
  // Public (no auth). Does NOT auto-increment download count.
  // ──────────────────────────────────────────────
  ...
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'vote' && method === 'POST') {
    ...
    if (existing) {
      ...
      const scoreDelta = vote === 'UP' ? 2 : -2 // switching from DOWN→UP = +2, UP→DOWN = -2
      ...
      await db.collection('resources').updateOne(
        { id: resourceId },
        { $inc: { voteScore: scoreDelta }, $set: { updatedAt: new Date() } }
      )
    } else {
      // New vote
      ...
      const scoreDelta = vote === 'UP' ? 1 : -1
      await db.collection('resources').updateOne(
        { id: resourceId },
        { $inc: { voteScore: scoreDelta, voteCount: 1 }, $set: { updatedAt: new Date() } }
      )
    }

    await invalidateResource(resourceId)

    const updatedResource = await db.collection('resources').findOne({ id: resourceId }, { projection: { _id: 0, voteScore: 1, voteCount: 1 } })
    return { data: { vote, voteScore: updatedResource.voteScore, voteCount: updatedResource.voteCount } }
  }
  ...
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'vote' && method === 'DELETE') {
    ...
    const scoreDelta = existing.vote === 'UP' ? -1 : 1
  ...
  if (path[0] === 'resources' && path.length === 3 && path[2] === 'download' && method === 'POST') {
    ...
    // Atomic increment — one per user per resource per 24h (dedup)
