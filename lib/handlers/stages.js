 ...
 * Returns a structured object with every signal value + eligibility.
 */
async function collectDistributionSignals(db, content) {
  ...
  const engagementScore = likeCount + (saveCount * 2) + (commentCount * 2)
  ...
  if (newStage === currentStage && !blocked) {
    if (currentStage === 0) {
      ...
      const eligible =
        ...
        signals.engagementScore >= r.minEngagement &&
...
function buildBlockedReason(signals, rules, transitionName) {
  ...
  if (rules.minEngagement && signals.engagementScore < rules.minEngagement)
    reasons.push(`engagement_${signals.engagementScore}_need_${rules.minEngagement}`)
