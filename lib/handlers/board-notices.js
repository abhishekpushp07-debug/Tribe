...
export async function handleBoardNotices(path, method, request, db) {
  ...
  if (path[0] === 'board' && path[1] === 'notices' && path.length === 2 && method === 'POST') {
    ...
    if (!seat && !isAdmin(user)) {
      return { error: 'Only board members or admins can create notices', code: ErrorCode.FORBIDDEN, status: 403 }
    }
    ...
    if (title.length > NOTICE_TITLE_MAX) return { error: `Title max ${NOTICE_TITLE_MAX} chars`, code: ErrorCode.VALIDATION, status: 400 }
    if (noticeBody.length > NOTICE_BODY_MAX) return { error: `Body max ${NOTICE_BODY_MAX} chars`, code: ErrorCode.VALIDATION, status: 400 }
    if (!ALLOWED_CATEGORIES.includes(category)) return { error: `Category must be: ${ALLOWED_CATEGORIES.join(', ')}`, code: ErrorCode.VALIDATION, status: 400 }
    if (!ALLOWED_PRIORITIES.includes(priority)) return { error: `Priority must be: ${ALLOWED_PRIORITIES.join(', ')}`, code: ErrorCode.VALIDATION, status: 400 }
  ...
  if (path[0] === 'board' && path[1] === 'notices' && path.length === 3 && method === 'GET') {
    ...
    if (notice.status === 'REMOVED') return { error: 'This notice has been removed', code: ErrorCode.GONE, status: 410 }

    const isOwner = viewer?.id === notice.creatorId
    const isAdminUser = viewer && isAdmin(viewer)

    if (notice.status === 'DRAFT' && !isOwner) return { error: 'Notice not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (notice.status === 'PENDING_REVIEW' && !isOwner && !isAdminUser) return { error: 'Notice not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (notice.status === 'REJECTED' && !isOwner && !isAdminUser) return { error: 'Notice not found', code: ErrorCode.NOT_FOUND, status: 404 }
  ...
  if (path[0] === 'board' && path[1] === 'notices' && path.length === 3 && method === 'PATCH') {
    ...
    const notice = await db.collection('board_notices').findOne({ id: noticeId })
    if (!notice) return { error: 'Notice not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (notice.creatorId !== user.id && !isAdmin(user)) return { error: 'Not authorized', code: ErrorCode.FORBIDDEN, status: 403 }
    if (['REMOVED'].includes(notice.status)) return { error: `Cannot edit notice with status ${notice.status}`, code: ErrorCode.INVALID_STATE, status: 400 }

    const body = await request.json()
    const updates = { updatedAt: new Date() }

    if (body.title !== undefined) {
      if (!body.title || body.title.length > NOTICE_TITLE_MAX) return { error: 'Title required', code: ErrorCode.VALIDATION, status: 400 }
      updates.title = body.title.trim()
    }
    if (body.body !== undefined) {
      if (!body.body || body.body.length > NOTICE_BODY_MAX) return { error: 'Body required', code: ErrorCode.VALIDATION, status: 400 }
    ...
    if (body.priority !== undefined) {
      if (!ALLOWED_PRIORITIES.includes(body.priority)) return { error: 'Invalid priority', code: ErrorCode.VALIDATION, status: 400 }
  ...
  if (path[0] === 'board' && path[1] === 'notices' && path.length === 4 && path[3] === 'pin' && method === 'POST') {
    ...
    if (!isAdmin(user)) {
      const seat = await db.collection('board_seats').findOne({ userId: user.id, status: 'ACTIVE' })
      if (!seat) return { error: 'Not authorized', code: ErrorCode.FORBIDDEN, status: 403 }
    }

    const noticeId = path[2]
    const notice = await db.collection('board_notices').findOne({ id: noticeId })
    if (!notice) return { error: 'Notice not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (notice.status !== 'PUBLISHED') return { error: 'Only published notices can be pinned', code: ErrorCode.INVALID_STATE, status: 400 }
    ...
    if (pinnedCount >= NOTICE_MAX_PINNED && !notice.pinnedToBoard) {
      return { error: `Max ${NOTICE_MAX_PINNED} pinned notices per college`, code: ErrorCode.LIMIT_EXCEEDED, status: 429 }
    }
  ...
  if (path[0] === 'board' && path[1] === 'notices' && path.length === 4 && path[3] === 'acknowledgments' && method === 'GET') {
    ...
    return { data: { items, pagination: { total }, total, noticeId } }
  }
  ...
  if (path[0] === 'me' && path[1] === 'board' && path[2] === 'notices' && path.length === 3 && method === 'GET') {
    ...
    return { data: { items: sanitizeDocs(notices), count: notices.length } }
  }
  ...
  if (path[0] === 'moderation' && path[1] === 'board-notices' && path.length === 4 && path[3] === 'decide' && method === 'POST') {
    ...
    if (typeof approve !== 'boolean') return { error: 'approve (boolean) required', code: ErrorCode.VALIDATION, status: 400 }
...
export async function handleAuthenticityTags(path, method, request, db) {
  ...
  if (path[0] === 'authenticity' && path[1] === 'tag' && path.length === 2 && method === 'POST') {
    ...
    if (!targetType || !ALLOWED_TAG_TARGETS.includes(targetType)) {
      return { error: `targetType must be: ${ALLOWED_TAG_TARGETS.join(', ')}`, code: ErrorCode.VALIDATION, status: 400 }
    }
    if (!targetId) return { error: 'targetId is required', code: ErrorCode.VALIDATION, status: 400 }
    if (!tag || !ALLOWED_TAGS.includes(tag)) {
      return { error: `tag must be: ${ALLOWED_TAGS.join(', ')}`, code: ErrorCode.VALIDATION, status: 400 }
    }
  ...
  if (path[0] === 'authenticity' && path[1] === 'tags' && path.length === 4 && method === 'GET') {
    ...
    return { data: { items, tags: items, summary, count: items.length } }
  }
  ...
  if (path[0] === 'authenticity' && path[1] === 'tags' && path.length === 3 && method === 'DELETE') {
    ...
    if (!tag) return { error: 'Tag not found', code: ErrorCode.NOT_FOUND, status: 404 }
