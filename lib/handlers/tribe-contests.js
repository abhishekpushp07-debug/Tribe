...
import { publishContestEvent, publishStandingsUpdate, ContestEventType, buildContestLiveStream, buildStandingsLiveStream, buildGlobalLiveStream } from '../contest-realtime.js'
import {
  ContestConfig, STATUS_TRANSITIONS, DEFAULT_SALUTE_DISTRIBUTION, SCORING_MODELS,
  canTransition, isAdmin, isContestAdmin, isJudge,
  computeContestScores, validateEntry, getSeasonStandings,
  sanitizeDoc, sanitizeDocs,
} from '../services/contest-service.js'

// Re-export for external consumers
const CONTEST_TYPES = ContestConfig.TYPES
const CONTEST_FORMATS = ContestConfig.FORMATS
const CONTEST_STATUSES = ContestConfig.STATUSES
const ENTRY_TYPES = ContestConfig.ENTRY_TYPES
const ENTRY_STATUSES = ContestConfig.ENTRY_STATUSES
const ENTRY_MODES = ContestConfig.ENTRY_MODES
const AUDIENCE_SCOPES = ContestConfig.AUDIENCE_SCOPES
const VOTE_TYPES = ContestConfig.VOTE_TYPES
const RESOLUTION_MODES = ContestConfig.RESOLUTION_MODES
...
export async function handleTribeContests(path, method, request, db) {
  ...
  if (path[0] === 'tribe-contests' && path.length === 1 && method === 'GET') {
    ...
    // Cache key for unauthenticated/standard queries
    const listCacheKey = `list:${q.page}:${q.limit}:${q.status || 'all'}:${q.contestType || 'all'}:${q.seasonId || 'all'}`
    const cached = await cache.get(CacheNS.CONTEST_LIST, listCacheKey)
    ...
    const [items, total] = await Promise.all([
      db.collection('tribe_contests').find(filter, { projection: { _id: 0 } }).sort(sortField).skip(skip).limit(q.limit).toArray(),
      db.collection('tribe_contests').countDocuments(filter),
    ])

    // Enrich with season names (batch lookup)
    const seasonIds = [...new Set(items.map(i => i.seasonId).filter(Boolean))]
    const seasons = seasonIds.length > 0
      ? await db.collection('tribe_seasons').find({ id: { $in: seasonIds } }, { projection: { _id: 0, id: 1, name: 1, year: 1 } }).toArray()
      : []
    ...
    const result = { items: enriched, pagination: { total, page: q.page, limit: q.limit, hasMore: q.page * q.limit < total }, page: q.page, limit: q.limit, total }
    await cache.set(CacheNS.CONTEST_LIST, listCacheKey, result, CacheTTL.CONTEST_LIST)
    return { data: result }
  }

  // ========================
  // GET /tribe-contests/live-feed — Global contest activity SSE
  // ========================
  if (path[0] === 'tribe-contests' && path[1] === 'live-feed' && path.length === 2 && method === 'GET') {
    return { raw: buildGlobalLiveStream(request, db) }
  }

  // ========================
  // GET /tribe-contests/:id/live — Live contest scoreboard SSE
  // ========================
  if (path[0] === 'tribe-contests' && path.length === 3 && path[2] === 'live' && method === 'GET') {
    return { raw: buildContestLiveStream(request, path[1], db) }
  }

  // ========================
  // GET /tribe-contests/seasons/:id/live-standings — Live season standings SSE
  // ========================
  if (path[0] === 'tribe-contests' && path[1] === 'seasons' && path.length === 4 && path[3] === 'live-standings' && method === 'GET') {
    return { raw: buildStandingsLiveStream(request, path[2], db) }
  }
  ...
  if (path[0] === 'tribe-contests' && path.length === 2 && method === 'GET'
    && !['leaderboard', 'results', 'entries', 'seasons', 'live-feed'].includes(path[1])) {
  ...
  if (path[0] === 'tribe-contests' && path.length === 3 && path[2] === 'enter' && method === 'POST') {
    ...
    if (contest.status !== 'ENTRY_OPEN') {
      return { error: 'Contest is not accepting entries', code: ErrorCode.CONTEST_NOT_OPEN, status: 400 }
    }

    // Check entry window
    const now = new Date()
    if (contest.entryEndAt && now > new Date(contest.entryEndAt)) {
      return { error: 'Entry period has ended', code: ErrorCode.ENTRY_PERIOD_ENDED, status: 400 }
    }
    ...
    if (!membership) return { error: 'You must belong to a tribe to enter', code: ErrorCode.NO_TRIBE, status: 400 }
    ...
    if (contest.eligibilityRules?.allowedTribeIds?.length > 0) {
      if (!contest.eligibilityRules.allowedTribeIds.includes(membership.tribeId)) {
        return { error: 'Your tribe is not eligible for this contest', code: ErrorCode.TRIBE_NOT_ELIGIBLE, status: 403 }
      }
    }
    ...
    if (userEntryCount >= (contest.maxEntriesPerUser || 1)) {
      return { error: 'Maximum entries reached for this contest', code: ErrorCode.MAX_ENTRIES_REACHED, status: 400 }
    }
    ...
    if (contest.maxEntriesPerTribe) {
      ...
      if (tribeEntryCount >= contest.maxEntriesPerTribe) {
        return { error: 'Your tribe has reached max entries for this contest', code: ErrorCode.TRIBE_MAX_ENTRIES, status: 400 }
      }
    }
    ...
    if (!entryType || !ENTRY_TYPES.includes(entryType)) {
      return { error: 'Valid entryType required', code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    if (contentId) {
      ...
      if (existingContent) {
        return { error: 'This content has already been submitted to this contest', code: ErrorCode.DUPLICATE_CONTENT, status: 409 }
      }
    }

    // Validate contentId exists in the correct collection
    if (contentId && ['reel', 'post', 'story'].includes(entryType)) {
      let contentExists = false
      if (entryType === 'reel') {
        contentExists = await db.collection('reels').findOne({ id: contentId, creatorId: user.id }, { projection: { _id: 0, id: 1 } })
      } else if (entryType === 'post') {
        contentExists = await db.collection('content_items').findOne({ id: contentId, authorId: user.id }, { projection: { _id: 0, id: 1 } })
      } else if (entryType === 'story') {
        contentExists = await db.collection('stories').findOne({ id: contentId, authorId: user.id }, { projection: { _id: 0, id: 1 } })
      }
      if (!contentExists) {
        return { error: `${entryType} content not found or not owned by you`, code: ErrorCode.NOT_FOUND, status: 404 }
      }
    } else if (['reel', 'post', 'story'].includes(entryType) && !contentId) {
      return { error: `contentId is required for ${entryType} entries`, code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    // RT: Broadcast entry submission
    publishContestEvent(contestId, ContestEventType.ENTRY_SUBMITTED, {
      entryId: entry.id, tribeId: membership.tribeId, tribeCode: membership.tribeCode,
      userId: user.id, entryType, contestName: contest.contestName,
    }).catch(() => {})
  ...
  if (path[0] === 'tribe-contests' && path.length === 3 && path[2] === 'entries' && method === 'GET') {
    ...
    return { data: { items: enriched, pagination: { total, page: q.page, limit: q.limit, hasMore: q.page * q.limit < total }, page: q.page, limit: q.limit, total } }
  }
  ...
  if (path[0] === 'tribe-contests' && path.length === 3 && path[2] === 'leaderboard' && method === 'GET') {
    ...
    return { data: { contestId, items: leaderboard, leaderboard, pagination: { total, page: q.page, limit: q.limit, hasMore: q.page * q.limit < total }, page: q.page, limit: q.limit, total, updatedAt: new Date().toISOString() } }
  }
  ...
  if (path[0] === 'tribe-contests' && path.length === 3 && path[2] === 'results' && method === 'GET') {
    ...
    if (contest.status !== 'RESOLVED') {
      return { error: 'Contest results not yet available', code: ErrorCode.NOT_RESOLVED, status: 400 }
    }

    const result = await db.collection('tribe_contest_results').findOne({ contestId })
    if (!result) return { error: 'Results not found', code: ErrorCode.NOT_FOUND, status: 404 }
  ...
  if (path[0] === 'tribe-contests' && path.length === 3 && path[2] === 'vote' && method === 'POST') {
    ...
    if (!['ENTRY_OPEN', 'ENTRY_CLOSED', 'EVALUATING'].includes(contest.status)) {
      return { error: 'Voting is not open for this contest', code: ErrorCode.VOTING_CLOSED, status: 400 }
    }

    // Check if voting is enabled for this contest
    if (contest.votingEnabled === false) {
      return { error: 'Voting is not enabled for this contest', code: ErrorCode.VOTING_DISABLED, status: 400 }
    }

    const body = await request.json()
    const { entryId, voteType } = body

    if (!entryId) return { error: 'entryId required', code: ErrorCode.VALIDATION, status: 400 }
    ...
    if (!entry) return { error: 'Entry not found or not eligible', code: ErrorCode.ENTRY_NOT_FOUND, status: 404 }

    // Anti-cheat: self-vote block (configurable)
    if (contest.selfVoteBlocked !== false && entry.userId === user.id) {
      return { error: 'Cannot vote for your own entry', code: ErrorCode.SELF_VOTE_BLOCKED, status: 403 }
    }
    ...
    if (existingVote) {
      return { error: 'You have already voted for this entry', code: ErrorCode.DUPLICATE_VOTE, status: 409 }
    }
    ...
    if (userVoteCount >= maxVotesPerContest) {
      return { error: `Maximum ${maxVotesPerContest} votes per contest reached`, code: ErrorCode.VOTE_CAP_REACHED, status: 400 }
    }
    ...
    try {
      ...
      if (e.code === 11000) {
        return { error: 'Duplicate vote', code: ErrorCode.DUPLICATE_VOTE, status: 409 }
      }
      throw e
    }

    // RT: Broadcast vote
    publishContestEvent(contestId, ContestEventType.VOTE_CAST, {
      entryId, tribeId: entry.tribeId, voterTribeId: voterMembership?.tribeId || null,
  ...
  if (path[0] === 'tribe-contests' && path.length === 3 && path[2] === 'withdraw' && method === 'POST') {
    ...
    if (!entry) return { error: 'Entry not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (['withdrawn', 'disqualified', 'locked'].includes(entry.submissionStatus)) {
      return { error: 'Entry cannot be withdrawn in current status', code: ErrorCode.INVALID_STATUS, status: 400 }
    }
...
export async function handleTribeContestAdmin(path, method, request, db) {
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 2 && method === 'POST') {
    ...
    if (!seasonId || !contestName) {
      return { error: 'seasonId and contestName required', code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    if (existingCode) return { error: 'Contest code already exists', code: ErrorCode.DUPLICATE_CODE, status: 409 }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 2 && method === 'GET') {
    ...
    return { data: { items: sanitizeDocs(items), pagination: { total, page: q.page, limit: q.limit, hasMore: q.page * q.limit < total }, page: q.page, limit: q.limit, total } }
  }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 3
    && !['publish', 'open-entries', 'close-entries', 'lock', 'resolve', 'disqualify', 'judge-score', 'compute-scores', 'cancel', 'dashboard', 'rules', 'recompute-broadcast'].includes(path[2])
    && method === 'GET') {
    const admin = await requireAuth(request, db)
    requireRole(admin, 'ADMIN', 'SUPER_ADMIN')
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'publish' && method === 'POST') {
    ...
    if (!canTransition(contest.status, 'PUBLISHED')) {
      return { error: `Cannot publish from status ${contest.status}`, code: ErrorCode.INVALID_TRANSITION, status: 400 }
    }
    ...
    // RT: Broadcast status transition
    publishContestEvent(contestId, ContestEventType.CONTEST_TRANSITION, {
      from: contest.status, to: 'PUBLISHED', contestName: contest.contestName,
    }).catch(() => {})
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'open-entries' && method === 'POST') {
    ...
    if (!canTransition(contest.status, 'ENTRY_OPEN')) {
      return { error: `Cannot open entries from status ${contest.status}`, code: ErrorCode.INVALID_TRANSITION, status: 400 }
    }
    ...
    // RT: Broadcast entries opened
    publishContestEvent(contestId, ContestEventType.CONTEST_TRANSITION, {
      from: contest.status, to: 'ENTRY_OPEN', contestName: contest.contestName,
    }).catch(() => {})
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'close-entries' && method === 'POST') {
    ...
    if (!canTransition(contest.status, 'ENTRY_CLOSED')) {
      return { error: `Cannot close entries from status ${contest.status}`, code: ErrorCode.INVALID_TRANSITION, status: 400 }
    }
    ...
    // RT: Broadcast entries closed
    publishContestEvent(contestId, ContestEventType.CONTEST_TRANSITION, {
      from: contest.status, to: 'ENTRY_CLOSED', contestName: contest.contestName,
    }).catch(() => {})
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'lock' && method === 'POST') {
    ...
    if (contest.status === 'ENTRY_CLOSED') {
      ...
      return { error: `Cannot lock from status ${contest.status}`, code: ErrorCode.INVALID_TRANSITION, status: 400 }
    }
    ...
    // RT: Broadcast contest locked
    publishContestEvent(contestId, ContestEventType.CONTEST_TRANSITION, {
      from: contest.status, to: 'LOCKED', contestName: contest.contestName,
    }).catch(() => {})
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'resolve' && method === 'POST') {
    ...
    if (contest.status !== 'LOCKED') {
      return { error: 'Contest must be locked before resolving', code: ErrorCode.INVALID_TRANSITION, status: 400 }
    }
    ...
    // RT: Broadcast contest resolved
    publishContestEvent(contestId, ContestEventType.CONTEST_RESOLVED, {
      contestName: contest.contestName,
      winnerTribeId: topPositions[0]?.tribeId,
      topPositions: topPositions.slice(0, 3),
      saluteCount: saluteEntries.length,
    }).catch(() => {})

    // RT: Broadcast standings update for the season
    if (contest.seasonId) {
      publishStandingsUpdate(contest.seasonId, {
        contestId,
        contestName: contest.contestName,
        affectedTribes: saluteEntries.map(se => ({ tribeId: se.tribeId, deltaSalutes: se.deltaSalutes, reasonCode: se.reasonCode })),
      }).catch(() => {})
    }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'disqualify' && method === 'POST') {
    ...
    if (!entryId || !reason) return { error: 'entryId and reason required', code: ErrorCode.VALIDATION, status: 400 }

    const entry = await db.collection('tribe_contest_entries').findOne({ id: entryId, contestId })
    if (!entry) return { error: 'Entry not found', code: ErrorCode.NOT_FOUND, status: 404 }

    if (entry.isDisqualified) {
      return { error: 'Entry already disqualified', code: ErrorCode.ALREADY_DISQUALIFIED, status: 409 }
    }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'judge-score' && method === 'POST') {
    ...
    if (!isJudge(judge)) {
      return { error: 'Unauthorized: judge role required', code: ErrorCode.FORBIDDEN, status: 403 }
    }
    ...
    if (!entryId || !rubricScores) {
      return { error: 'entryId and rubricScores required', code: ErrorCode.VALIDATION, status: 400 }
    }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'compute-scores' && method === 'POST') {
    ...
    // Delegate to the canonical service function that pulls real content engagement
    const result = await computeContestScores(db, contestId)
    if (result.error) return { error: result.error, code: ErrorCode.NOT_FOUND, status: 404 }

    return { data: { message: `Scores computed for ${result.computed} entries`, contestId, totalScored: result.totalScored } }
  }

  // ========================
  // POST /admin/tribe-contests/:id/recompute-broadcast — Compute scores AND broadcast rank changes
  // ========================
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'recompute-broadcast' && method === 'POST') {
    const admin = await requireAuth(request, db)
    requireRole(admin, 'ADMIN', 'SUPER_ADMIN')
    ...
    // Get old leaderboard for rank change detection
    const oldScores = await db.collection('tribe_contest_scores')
      .find({ contestId })
      .sort({ finalScore: -1 })
      .toArray()
    const oldRanks = Object.fromEntries(oldScores.map((s, i) => [s.entryId, i + 1]))

    // Trigger regular compute (reuse existing logic internally)
    const computeReq = new Request(request.url, { method: 'POST', headers: request.headers })
    // We manually call the compute scores logic inline
    ...
    for (const entry of entries) {
      const judgeScores = await db.collection('contest_judge_scores').find({ contestId, entryId: entry.id }).toArray()
      const voteCount = await db.collection('contest_votes').countDocuments({ contestId, entryId: entry.id })
      let judgeAvg = judgeScores.length > 0 ? judgeScores.reduce((s, js) => s + js.totalScore, 0) / judgeScores.length : 0
      const engagementScore = voteCount * 2
      const penaltyScore = entry.isDisqualified ? 100 : 0
      const weights = scoringModel.weights
      let finalScore = 0
      if (contest.scoringModelId === 'scoring_judge_only_v1') finalScore = judgeAvg - penaltyScore
      else if (contest.scoringModelId === 'scoring_participation_v1') finalScore = 10 + engagementScore - penaltyScore
      else finalScore = (judgeAvg * (weights.judge || 0.35)) + (engagementScore * (weights.valid_likes || 0.10)) + (judgeAvg * 0.1) + 5 * 0.05 - penaltyScore
      finalScore = Math.round(finalScore * 100) / 100

      await db.collection('tribe_contest_scores').updateOne(
        { contestId, entryId: entry.id },
        { $set: { tribeId: entry.tribeId, userId: entry.userId, judgeScore: judgeAvg, engagementScore, qualityScore: judgeAvg, participationScore: 10, penaltyScore, finalScore, scoringVersion: contest.scoringModelId, lastComputedAt: now, updatedAt: now },
    ...
    // Recompute ranks
    const newScores = await db.collection('tribe_contest_scores').find({ contestId }).sort({ finalScore: -1, lastComputedAt: 1 }).toArray()
    const rankChanges = []
    for (let i = 0; i < newScores.length; i++) {
      const newRank = i + 1
      const oldRank = oldRanks[newScores[i].entryId] || null
      await db.collection('tribe_contest_scores').updateOne({ id: newScores[i].id }, { $set: { rank: newRank } })
      if (oldRank && oldRank !== newRank) {
        rankChanges.push({ entryId: newScores[i].entryId, tribeId: newScores[i].tribeId, oldRank, newRank, direction: newRank < oldRank ? 'up' : 'down', delta: Math.abs(oldRank - newRank) })
      }
    }

    // RT: Broadcast score update + rank changes
    publishContestEvent(contestId, ContestEventType.SCORE_UPDATED, {
      contestName: contest.contestName, entriesScored: entries.length,
    }).catch(() => {})

    if (rankChanges.length > 0) {
      publishContestEvent(contestId, ContestEventType.RANK_CHANGED, {
        contestName: contest.contestName, changes: rankChanges,
      }).catch(() => {})
    }

    return { data: { message: `Scores recomputed + broadcast for ${entries.length} entries`, rankChanges, totalScored: entries.length } }
  }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'cancel' && method === 'POST') {
    ...
    if (contest.status === 'RESOLVED') {
      return { error: 'Cannot cancel a resolved contest', code: ErrorCode.INVALID_TRANSITION, status: 400 }
    }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path[2] === 'rules' && path.length === 3 && method === 'POST') {
    ...
    if (!contestId || !ruleType || !rulePayload) {
      return { error: 'contestId, ruleType, and rulePayload required', code: ErrorCode.VALIDATION, status: 400 }
    }
