...
const HLS_SEGMENT_DURATION = 4  // Shorter segments = faster start
...
export async function processVideo(db, asset) {
  ...
  try {
    ...
    // Best URL: HLS > quality 720p > 480p > 360p > ultrafast > faststart > original
...
export async function getProcessingStatus(db, mediaId) {
  ...
  return {
    ...
    recommended: {
      primary: asset.hlsUrl || v.hls?.url || v['720p']?.url || v['480p']?.url || v.ultrafast_720p?.url || v.faststart?.url || asset.publicUrl,
      fallback: v['480p']?.url || v['360p']?.url || v.ultrafast_720p?.url || v.faststart?.url || asset.publicUrl,
