  ...
  return posts.map(p => {
    ...
    if (Array.isArray(clean.media) && clean.media.length > 0) {
      resolvedMedia = clean.media.map(m => {
        ...
        // Use best available playback URL: HLS > 720p > 480p > 360p > faststart > CDN original
        const variants = asset.variants || {}
        const bestUrl = (isVideo && (asset.hlsUrl || variants.hls?.url || variants['720p']?.url || variants['480p']?.url || variants['360p']?.url || variants.faststart?.url))
        ...
        return {
          ...
          playbackUrl: isVideo ? bestUrl : undefined,
